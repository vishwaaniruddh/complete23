...
$bank = "PNB"; 
$sql_qry = "select ATMID,SN,SiteAddress,DVRIP,PanelIP,RouterIp,ATMID_2,ATMID_3,live_date,Zone,State from sites where Customer='" . $client . "' and Bank='" . $bank . "' and live='Y' AND SN IN (SELECT SN from `newnetwork_report` WHERE month_date='".$mon_date."' AND month='".$month."' AND year='".$year."' AND today_tot_hit=0) limit 100";
			...
			if($_view == 1){
			    ...
			    if(mysqli_num_rows($newnet_sql_res)>0){
				   ...
				   // echo $id;die;
					...
					/*$set_sql = "INSERT INTO `newnetwork_report`( `SN`, `month_date`, `month`, `year`, `uptime`, `downtime`, `created_at`, `today_tot_hit` ) VALUES ('" . $SN . "','" . $mon_date . "','" . $month . "','" . $year . "','" . $total_uptime . "','" . $total_downtime . "','" . $created_at . "', '".$nowtime_hour."') ";
						$set_result = mysqli_query($con, $set_sql);
						$count = $count + 1; */
				}
