						 ...
						 if(mysqli_num_rows($sql)){
							while($sql_result = mysqli_fetch_assoc($sql)){
								...
								if($_view == 1){
									...
									/*	
									$network_routersql = mysqli_query($con,"select device,status from network_history where site_id ='".$sn."' and device='R'"); 
									$router_status_total = mysqli_num_rows($network_routersql); 
									$network_routerstatussql = mysqli_query($con,"select device,status from network_history where site_id ='".$sn."' and device='R' and status='OK'"); 
									$router_status_online = mysqli_num_rows($network_routerstatussql); 
									
									$network_dvrsql = mysqli_query($con,"select device,status from network_history where site_id ='".$sn."' and device='D'"); 
									$dvr_status_total = mysqli_num_rows($network_dvrsql); 
									$network_dvrstatussql = mysqli_query($con,"select device,status from network_history where site_id ='".$sn."' and device='D' and status='OK'"); 
									$dvr_status_online = mysqli_num_rows($network_dvrstatussql); 
									
									$network_panelsql = mysqli_query($con,"select device,status from network_history where site_id ='".$sn."' and device='P'"); 
									$panel_status_total = mysqli_num_rows($network_panelsql); 
									$network_panelstatussql = mysqli_query($con,"select device,status from network_history where site_id ='".$sn."' and device='P' and status='OK'"); 
									$panel_status_online = mysqli_num_rows($network_panelstatussql); 
							...
							$check_network_report_list = mysqli_query($con,"select id from network_report_list where SN='".$sn."'");
                            ...
                            if(mysqli_num_rows($check_network_report_list)==0){
								...
								$insert_sql = "insert into network_report_list (SN,ATMID,SiteAddress,router_status,router_ip,router_lastcommunication,router_online_percent,dvr_status,dvr_ip,dvr_lastcommunication,dvr_online_percent,panel_status,panel_ip,panel_lastcommunication,panel_online_percent,created_at,last_action_date)
								              values('".$sn."','".$atm_id."','".$site_address."','".$router_status."','".$routerip."','".$routerlast_communication."','".$routeronlinepercent."','".$dvr_status."','".$dvrip."','".$dvrlast_communication."','".$dvronlinepercent."','".$panel_status."','".$panelip."','".$panellast_communication."','".$panelonlinepercent."','".$created_at."','".$created_date."')";
						        
								$result=mysqli_query($con,$insert_sql) ;
							}else{
								$net_rep_list = mysqli_fetch_assoc($check_network_report_list);
								$tb_id = $net_rep_list['id'];
								$updatesql = "update network_report_list SET SN='".$sn."',ATMID='".$atm_id."',SiteAddress='".$site_address."',router_status='".$router_status."',router_ip='".$routerip."',router_lastcommunication='".$routerlast_communication."',router_online_percent='".$routeronlinepercent."',dvr_status='".$dvr_status."',dvr_ip='".$dvrip."',dvr_lastcommunication='".$dvrlast_communication."',dvr_online_percent='".$dvronlinepercent."',panel_status='".$panel_status."',panel_ip='".$panelip."',panel_lastcommunication='".$panellast_communication."',panel_online_percent='".$panelonlinepercent."',last_action_date='".$created_date."' where id='".$tb_id."')";
