...
$month = 6;
$bank = "PNB"; 
$sql_qry = "select ATMID,SN,SiteAddress,DVRIP,PanelIP,RouterIp,ATMID_2,ATMID_3,live_date,Zone,State from sites where Customer='" . $client . "' and Bank='" . $bank . "' and live='Y' AND SN NOT IN (SELECT SN from `newdowntime_report_2` WHERE month_date='".$mon_date."') limit 0,250";
//echo $sql_qry;die;
...
if(mysqli_num_rows($sql)){
	...
	while($sql_result = mysqli_fetch_assoc($sql)){
			...
			if($_view == 1){
               ...
               // echo "total_downtime_day : ".$total_downtime_day." & ";
				if($total_downtime == 0){
					$system_down_from = "";
				}else{
					if($system_down_from!=''){
					//	echo 'system_down_from : '.$system_down_from.' & ';
						...
						//echo "split_time_only : ".$split_time_only[0].' & ';
				...
				//echo '$system_down_up_to : '.$system_down_up_to;
				//die;
				$site_up_with_tat = 24 - $total_downtime;
				$reason = "";
				$bank_advised_stop_surveillance = "N";
				$power_supply_inadequate_battery = "N" ; $operator_comments = ""; $total_downtime_current_month = 0;  
				
				$dwn_qry_sql = "SELECT total_downtime_current_month FROM `newdowntime_report_2` WHERE SN='".$SN."' AND month='".$month."' order by id DESC limit 1";
				//echo $net_qry_sql;die;
