...
<div class="table-responsive">
							...
							while($sql_result = mysqli_fetch_assoc($sql)){
								...
								if($_view == 1){
									...
									if(mysqli_num_rows($aisql)>0){
										...
										echo $routerlast_communication.'_';
										...
										//echo mysqli_num_rows($check_network_report_list);die;
										if(mysqli_num_rows($check_network_report_list)==0){ 
											...
											//echo $tb_last_action_date.'_'.$yesterday;die;
											if($tb_last_action_date>$yesterday){
											 ...
											 // echo '2';die;
											  // $test_sql = "select device,status from network_history where site_id ='".$sn."' AND CAST(rectime AS DATE)>'".$tb_last_action_date."' AND CAST(rectime AS DATE)<='".$today."'";
											  // echo $test_sql;die;
											   $networksql = mysqli_query($con,"select device,status,rectime from network_history where site_id ='".$sn."' AND CAST(rectime AS DATE)>'".$tb_last_action_date."' AND CAST(rectime AS DATE)<='".$today."'"); 
											   $will_update = 1;
											   $is_networksql = 1;
											}
							        ...
							        if($check_network_report_list_cnt==0){
										...
										/*	$updatesql = "update network_report_list SET SN='".$sn."',Customer='".$customer."',Bank='".$bank."',ATMID='".$atm_id."',SiteAddress='".$site_address."',router_status='".$router_status."',router_ip='".$routerip."',router_lastcommunication='".$routerlast_communication."',router_online_count='".$router_status_online."',router_tot_count='".$router_status_total."',router_online_percent='".$routeronlinepercent."',
														  ...
														  panel_status='".$panel_status."',panel_ip='".$panelip."',panel_lastcommunication='".$panellast_communication."',panel_online_count='".$panel_status_online."',panel_tot_count='".$panel_status_total."',panel_online_percent='".$panelonlinepercent."',last_action_date='".$yesterday."',live='".$live."' where id='".$tb_id."'";
											echo $updatesql;die;
				  ...
				  <td><?php echo $panelonlinepercent;?></td>
									...
									<?php  echo $router_status.'_';if($count==2){die;} }
							  }
						   }
						 }
						  CloseCon($con);
