...
$banks = explode(",",$_SESSION['bankname']);
	...
	*/
	...
	//$file_list = ftp_nlist($ftp_conn, "AI_Feed/A1009510");
	...
	if(count($all_atm)>0){
		for($i=0;$i<count($all_atm);$i++){
			...
			if($all_atm[$i]!='desktop.ini'){
				...
				$atmfile_list_share1 = ftp_nlist($ftp_conn_1,$path);
				if(count($atmfile_list_share1)>0){
				...
				$mainfile_list_share = ftp_nlist($ftp_conn_1,$_time_folder); 
				//echo '<pre>';print_r($mainfile_list_share);echo '</pre>';
				$last_folder_arr = array();
				if(count($mainfile_list_share)>0){
					for($z=0;$z<count($mainfile_list_share);$z++){
