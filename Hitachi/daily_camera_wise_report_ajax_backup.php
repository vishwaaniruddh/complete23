                    ...
                    <table id="order-listing" class="table">
                      <thead >
                        <tr>
							...
							<th>Site Type</th>
							<th>Camera</th>
							<?php for($i=0;$i<count($_dt_range);$i++){ ?>
							<th><?php echo $_dt_range[$i]." (Uptime in Hrs)"; ?></th>
							<?php } ?>
							<th>Total Up in Hrs</th>
							<th>Total Down in Hrs</th>					
                        </tr>
                      </thead>
                      <tbody>
						 ...
						 if(mysqli_num_rows($sql)){
							while($sql_result = mysqli_fetch_assoc($sql)){
								...
								if($_view == 1){
                                    ...
                                    //echo '<pre>';print_r($_cam1_arr);echo '</pre>';die; 
									...
									?>
									   <tr>
										   ...
										   <td><?php echo '-';?></td>
										   <td>Lobby Camera</td>
										   <?php $_lobby_tot_uptime = 0;
										         for($i=0;$i<count($_dt_range);$i++){ 
												    $_lobby_cam_dt = $_dt_range[$i];
                                                    $_lobby_tot_uptime = $_lobby_tot_uptime + $_cam1_arr[$_lobby_cam_dt];
											?>
						                   <td><?php echo $_cam1_arr[$_lobby_cam_dt];?></td>
										   <?php } $total_cam1_downtime = $total_timemonth - $_lobby_tot_uptime; ?>
										   <td><?php echo $_lobby_tot_uptime;?></td>
										   <td><?php echo $total_cam1_downtime;?></td>
										</tr>
										...
										<tr>
									      <td><?php echo $count;?></td>
										   ...
										   <td><?php echo $site_address;?></td>
										   <td><?php echo '-';?></td>
										   <td>Outside ATM Room Camera</td>
										   <?php $_outside_tot_uptime = 0;
										         for($i=0;$i<count($_dt_range);$i++){ 
												    $_outside_cam_dt = $_dt_range[$i];
                                                    $_outside_tot_uptime = $_outside_tot_uptime + $_cam2_arr[$_outside_cam_dt];
											?>
						                   <td><?php echo $_cam2_arr[$_outside_cam_dt];?></td>
										   <?php } $total_cam2_downtime = $total_timemonth - $_outside_tot_uptime; ?>
										   <td><?php echo $_outside_tot_uptime;?></td>
										   <td><?php echo $total_cam2_downtime;?></td>
										</tr>
										...
										<tr>
										   ...
										   <?php $_backroom_tot_uptime = 0;
										         for($i=0;$i<count($_dt_range);$i++){ 
												    $_backroom_cam_dt = $_dt_range[$i];
                                                    $_backroom_tot_uptime = $_backroom_tot_uptime + $_cam2_arr[$_backroom_cam_dt];
											?>
						                   <td><?php echo $_cam3_arr[$_backroom_cam_dt];?></td>
										   <?php } $total_cam3_downtime = $total_timemonth - $_backroom_tot_uptime; ?>
										   <td><?php echo $_backroom_tot_uptime;?></td>
										   <td><?php echo $total_cam3_downtime;?></td>
										</tr>
										...
										<tr>
										   ...
										   <td><?php echo $site_address;?></td>
										   <td><?php echo '-';?></td>
										   <td>PIN Hole Camera</td>
										   <?php $_pin_tot_uptime = 0;
										         for($i=0;$i<count($_dt_range);$i++){ 
												    $_pin_cam_dt = $_dt_range[$i];
                                                    $_pin_tot_uptime = $_pin_tot_uptime + $_cam2_arr[$_pin_cam_dt];
											?>
						                   <td><?php echo $_cam4_arr[$_pin_cam_dt];?></td>
										   <?php } $total_cam4_downtime = $total_timemonth - $_pin_tot_uptime; ?>
										   <td><?php echo $_pin_tot_uptime;?></td>
										   <td><?php echo $total_cam4_downtime;?></td>
										</tr>
