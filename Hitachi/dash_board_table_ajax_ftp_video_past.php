		...
		for($i=0;$i<count($all_atm);$i++){
			...
			if($all_atm[$i]!='desktop.ini'){
				...
				if(count($atmfile_list_share)>0){
							...
							if($filepath_explode[2]!='desktop.ini'){
							   ...
							   if(count($filepath_explode)==3){
									...
									if(count($datefile_explode)>0){
										for($b=0;$b<count($datefile_explode);$b++)
					...
					$_time_folder = $path."/".$start_folder;
					...
					if(count($mainfile_list_share)>0){
						for($z=0;$z<count($mainfile_list_share);$z++){
							...
							if(count($folderfilepath_explode)==4){
								if($folderfilepath_explode[3]!='desktop.ini'){
							       array_push($last_folder_arr,$folderfilepath_explode[3]);
								}
							}
						}
					}
					//echo '<pre>';print_r($last_folder_arr);echo '</pre>';die;
					$strtcreatedatetime = ""; 
					if(count($last_folder_arr)>0){
						$min_folder = min($last_folder_arr);
						$_thisfolder_strt = (int)$min_folder;
						$_thisfolder_end = $_thisfolder_strt + 1;
						//echo $_thisfolder_strt."-".$_thisfolder_end;die;
						...
						$strtcreatedatetime = $start_folder.', '.$sentence;
					}
					...
					//echo count($year_arr)."-".count($month_arr)."-".count($date_arr);die;
					if(count($year_arr)>0 && count($month_arr)>0 && count($date_arr)>0){
						...
						//echo '<pre>';print_r($last_folder_arr);
						//echo '</pre>';echo max($last_folder_arr);
						if(count($last_folder_arr)>0){
							...
							//echo $createdatetime;
						   // echo '<pre>';print_r($videofile_list_share);echo '</pre>';die;
							//echo count($videofile_list_share);die;
							if(count($videofile_list_share)==1){
								$videofolderfilepath_explode = explode('/',$videofile_list_share[0]);
								if($videofolderfilepath_explode[3]=='desktop.ini'){
								//	echo $max_folder."__";
								...
								if(count($videofile_list_share)>0){
									//if($videofile_list_share[$z]!='desktop.ini'){
										$file_hr_arr = array();
										$file_sec_arr = array();
					...
					$data_arr['start_created_date'] = $strtcreatedatetime;
