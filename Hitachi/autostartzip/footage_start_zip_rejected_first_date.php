...
function create_zip($files = array(),$destination = '',$overwrite = false) {
	 ...
	 */	
       ...
       // $newfolder_path = "Footage_Upload/Rejected/".$mon."/".$dt."/N2789300  19-06-2023 09.33 TO 10.03"; 	
		...
		if(count($newfile_list)>0){ 
			...
			for($j=0;$j<count($newfile_list);$j++){
				...
				if(count($split_atm_image)==5){
					...
					$check_footage = "SELECT * FROM `footage_details_available_status` WHERE month='".$mon."' AND dt='".$dt."' AND atmid='".$_atmid_imageval."' AND footage_type='".$footage_type."'";
