...
function create_zip($files = array(),$destination = '',$overwrite = false) {
	 ...
	 */	
       // $fileName = $atm_val.'.zip';
        ...
        $newfolder_path = "Footage_Upload/24Hrs/".$mon."/".$dt; 	
			...
			for($j=0;$j<count($newfile_list);$j++){
				...
				if(count($split_atm_image)==5){
					...
					if(mysqli_num_rows($check_footage_res)==0){
						...
						$local_file_dir = "E:/Footage/24Hrs/".$_temp_folder_path;
						...
						if(count($finalfile_list)>0){ 
							...
							if(count($imagesdata_array)>0){
							    ...
							    $set_sql = "INSERT INTO `footage_details_available_new`( `month`, `dt`,`atmid`, `created_at`, `updated_at`, `created_by`, `footage_type`,`local_file_dir`,`local_file`) VALUES ('".$mon."','".$dt."','".$_atmid_imageval."','".$current_datetime."','".$current_datetime."','24','".$footage_type."','".$local_file_dir."','".$local_file_array."') ";
							...
							//array_push($finalimagesdata_array,$imagesdata_array);
							    ...
							    $local_file_array = json_encode($imagesdata_array);
							    $footage_status = 0;
							    $set_sql = "INSERT INTO `footage_details_available_status`( `month`, `dt`,`atmid`, `created_at`, `updated_at`, `created_by`, `footage_type`,`local_file_dir`,`local_file`,`footage_status`) VALUES ('".$mon."','".$dt."','".$_atmid_imageval."','".$current_datetime."','".$current_datetime."','24','".$footage_type."','".$local_file_dir."','".$local_file_array."','".$footage_status."') ";
						        //echo $set_sql;die;
								$set_result = mysqli_query($con,$set_sql);
						} 
				    }
