...
$bank = "PNB"; 
$mon_date = $today;
$sql_qry = "select ATMID,SN,SiteAddress,DVRIP,PanelIP,RouterIp,ATMID_2,ATMID_3,live_date,Zone,State from sites where Customer='" . $client . "' and Bank='" . $bank . "' and live='Y' AND SN NOT IN (SELECT SN from `newnetwork_report_test` WHERE month_date='".$mon_date."' AND month='".$month."' AND year='".$year."')";
//echo $sql_qry;die;
...
if(mysqli_num_rows($sql)){
	while($sql_result = mysqli_fetch_assoc($sql)){
		...
		if($_view == 1){
			...
			$total_time = 0;$net_his_not = 0;
			$tot_cnt = 0;
	
		    $set_sql = "INSERT INTO `newnetwork_report_test`( `SN`, `month_date`, `month`, `year`, `uptime`, `downtime`, `created_at`) VALUES ('" . $SN . "','" . $mon_date . "','" . $month . "','" . $year . "','" . $total_time . "','" . $net_his_not . "','" . $created_at . "') ";
