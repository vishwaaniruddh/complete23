...
.tooltip:hover .tooltiptext {
  visibility: visible;
         ...
         <?php include('top-navbar.php');
				...
				$getstatussql = "SELECT * FROM `footage_details_available_status` WHERE month='".$mon."' AND dt='".$dt."' AND footage_type='Rejected'"; 
				//echo $getimagesql;
				$getstatusdata = mysqli_query($con,$getstatussql);
				$checkstatus_arr = array();
				//echo mysqli_num_rows($getimagesdata);die;
				if(mysqli_num_rows($getstatusdata)>0){
					while($chfoot_data = mysqli_fetch_assoc($getstatusdata)){
						$fol_atmid = $chfoot_data['atmid'];
						$seq = $mon."_".$dt."_".$fol_atmid;
