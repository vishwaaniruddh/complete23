...
if($atmid!=''){
	$sitesql = mysqli_query($con,"select * from dvr_health where atmid='".$atmid."' and login_status=1 and status=1");
}else{
	if($bank!=''){
		if($circle!=''){
	...
	//$testsql = "SELECT * FROM dvr_health WHERE atmid IN (".$atmidarray.") and login_status=1 and status=1";
	$testsql ="SELECT * FROM `dvr_health` WHERE atmid IN (".$atmidarray.") AND hdd is NOT NULL AND hdd!=''";
