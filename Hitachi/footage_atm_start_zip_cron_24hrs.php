	...
	//	echo '<pre>';print_r($newfile_list);echo '</pre>';die;
		if(count($newfile_list)>0){ 
			...
			for($j=0;$j<count($newfile_list);$j++){
				...
				//echo '<pre>';print_r($split_atm_image);echo '</pre>';
				if(count($split_atm_image)==4){
					$imagesdata_array = array(); 
					$_atmid_imageval = $split_atm_image[3];
					...
					if(mysqli_num_rows($check_footage_res)==0){
						...
						if(count($finalfile_list)>0){ 
							for($i=0;$i<count($finalfile_list);$i++){
								...
								//echo '<pre>';print_r($finalsplit_atm_image);echo '</pre>';
								if(count($finalsplit_atm_image)==5){
									$final_atmid_imageval = $finalsplit_atm_image[4]; 
			...
			echo '<pre>';print_r($finalimagesdata_array);echo '</pre>';*/
		}
		$insert_data = [];
