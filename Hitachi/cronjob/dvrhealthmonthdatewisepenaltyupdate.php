<?php  include('db_connection.php'); $con = OpenCon();
    function lastcommunicationdiff($datetime1,$datetime2){
      ...
      $end = '2022-03-31';	  
	  $month = 3;
	  $sql = mysqli_query($con,"select SN,id from dvr_health_site_monthwise where offline_percent>0 AND offline_percent<100 AND month='3' AND year='2022' AND total_down_time is null");
	  if(mysqli_num_rows($sql)){
		while($sitesql_result = mysqli_fetch_assoc($sql)){
			$id = $sitesql_result['id'];
			$SN = $sitesql_result['SN'];
			$testsql = "SELECT status,rectime FROM network_history WHERE site_id ='".$SN."' AND CAST(rectime AS DATE)>='".$start."' 
