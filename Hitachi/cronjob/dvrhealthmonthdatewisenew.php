<?php  include('db_connection.php'); $con = OpenCon();
	 ...
	 $sql = mysqli_query($con,"select SN,id from dvr_health_site_monthwise where offline_percent=0 AND online_percent=0 AND month='3' AND year='2022' AND total_down_time is null");
	  
	  if(mysqli_num_rows($sql)){
		while($sitesql_result = mysqli_fetch_assoc($sql)){
			$SN = $sitesql_result['SN']; 
			//$siteaddress = $sitesql_result['SiteAddress'];$ip = $sitesql_result['DVRIP'];
			$testsql = "SELECT status FROM network_history WHERE site_id ='".$SN."' AND CAST(rectime AS DATE)>='".$start."' 
						...
						//echo mysqli_num_rows($dvrhis_query);die;
						$dvr_online_count = 0;
						$dvr_offline_count = 0;
