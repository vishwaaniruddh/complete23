...
$("#show_detail").click(function(){ 
	...
	//get_ticketview();
        ...
        function getPanel_Detail(){
			...
			$("#load").show();
			$.ajax({
				url: "dash_board_table_ajax_ftp_video_cloud.php", 
				type: "POST",
					   ...
					   */
					   ...
					   if(data.length>0){
						   ...
						   for(i=0;i<data.length;i++){
							   ...
							   tbody += '<tr><td>'+count+'</td><td>'+data[i].atm_id+'</td><td>'+data[i].createdatetime+'</td><td>'+data[i].file_name+'</td></tr>';
						     count++;
						   }
						   tbody += '</tbody>';
						   var thead = '<thead><tr><th>S.N</th><th>ATM-ID</th><th>Last Uploaded Date Time</th><th>Filename</th> </tr></thead>';
