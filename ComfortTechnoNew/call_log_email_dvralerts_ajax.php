...
<?php 
 ...
 $created_by = $_SESSION['userid'];
  ...
  $check_call_log_emailsql = mysqli_query($con,"select * from call_log_dvr_alerts_email where action_date = '".$created_date."'");
 ...
 //echo $_SESSION['userid'];die;
    ...
    if($totalsite>0){
	    while($sql_result = mysqli_fetch_assoc($sql)){
			...
			$select_qry = "select * from call_log_dvr_alerts where ATMID='".$atmid."' AND current_status=0 AND call_log_aging<=3";
			$select_data = mysqli_query($con,$select_qry);
			if(mysqli_num_rows($select_data)==0){
				$insert_sql = "insert into call_log_dvr_alerts (ticket_id,ATMID,current_status,autoclose,current_remark,created_by,created_at,updated_by,updated_at,action_date)
				...
				if($result) {
				  ...
				  $insert_sql_history = "insert into call_log_dvr_alerts_history (call_log_id,current_status,current_remark,updated_by,updated_at)
				...
				$id = $res_data['id'];
				$created_at_dt_time = $res_data['created_at'];
				$call_log_aging = lastcommunicationdiff($created_at_dt_time);
				$update_sql = "update call_log_dvr_alerts SET call_log_aging='".$call_log_aging."' where id='".$id."'";
		    ...
		    $call_log_select_qry = "select * from call_log_dvr_alerts where current_status=0";
			...
			while($call_log_res_data = mysqli_fetch_assoc($call_log_select_data)){
				...
				$update_sql = "update call_log_dvr_alerts SET call_log_aging='".$call_log_call_log_aging."' where id='".$call_log_id."'";
				$call_log_update_sql_qry = mysqli_query($con, $call_log_update_sql);
				if($call_log_call_log_aging>3){
					$call_log_make_status = 1;
					$call_log_hdd = 'Close';
					$call_log_updatesql = "update call_log_dvr_alerts SET current_status=1,autoclose=1,current_remark='".$call_log_hdd."',updated_by='".$created_by."',updated_at='".$created_at."' WHERE id='".$call_log_id."'";
					if(mysqli_query($con,$call_log_updatesql)){
						$call_log_insertcallhissql = "insert into call_log_dvr_alerts_history (call_log_id,current_status,current_remark,updated_by,updated_at)
	    ...
	    $insertemail_sql = "insert into call_log_dvr_alerts_email (action_date,action_datetime)
