...
if($atmid!=''){
$sql = mysqli_query($con,"select cam1,atmid,last_communication,ip,cdate from all_dvr_live where atmid='".$atmid."' and status='0'"); 
}else{
	if($bank!=''){
	...
	$offlinetestsql = "SELECT cam1,atmid,last_communication,IPAddress,cdate FROM all_dvr_live WHERE atmid IN (".$atmidarray.") and status='0'";
	//echo $offlinetestsql;
