...
if($atmid!=''){
	if($status=='all'){
$sql = mysqli_query($con,"select * from footage_request_1 where atmid='".$atmid."' order by id desc"); 		
	}else{
$sql = mysqli_query($con,"select * from footage_request_1 where atmid='".$atmid."' and status='".$status."' order by id desc"); 
	}
