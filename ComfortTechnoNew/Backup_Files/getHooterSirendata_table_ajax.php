...
//echo '<pre>';print_r($panelidarray);echo '</pre>';die;  $org_id,$panel_id,$group_id,$access_token,$con
$getZone_details = json_decode(getHooterSirenStatus($org_id,$panelidarray1,$group_id,$access_token,$con),true);
...
<div class="table-responsive">
                    <table id="order-listing" class="table">
                        ...
                        $count = 0; 
						 if(count($getZone_details['result'])>0){
							...
							foreach($zone_res_data as $zone_res_value){
								$zone_data_arr = $zone_res_value['latest_config'];
								$mac_id = $zone_data_arr['mac_id'];
								$panel_name = $zone_data_arr['panel_name'];
								$split_panel = explode('_',$panel_name);
								$atm_id = $split_panel[0];
								$_view = 0;
								if(count($_circle_name_array)==0){
									$_view = 1;
								}else{
								...
								if($_view == 1){
									$count++;
									if($zone_data_arr['hooter_status']=='0'){
										   $_stat = 0;
										   $_st_txt = 'On';
									   ...
									   if($zone_data_arr['siren_status']=='0'){
										   $_arm_st = 0;
										   $_arm_st_txt = 'On';
										   ...
										   $_arm_st = 1;
										   $_arm_st_txt = 'Off';
										   $_arm_stat_class = 'badge-outline-danger';
									   }
					  ...
					  ?>
                                ...
                                <td>
								    <div class="badge <?php echo $_stat_class;?> badge-pill"><?php echo $_st_txt;?></div>
								</td>
								<td>
								    <div class="badge <?php echo $_arm_stat_class;?> badge-pill"><?php echo $_arm_st_txt;?></div>
								</td>
								</tr>
