...
<?php 
	...
	if($atmid!=''){
		if($status=='all'){
	        $sql = mysqli_query($con,"select * from footage_request_logic where atmid='".$atmid."' order by id desc"); 		
		}else{
	        $sql = mysqli_query($con,"select * from footage_request_logic where atmid='".$atmid."' and status='".$status."' order by id desc"); 
		}
		...
		if($status=='all'){
			$onlinetestsql = "SELECT * FROM footage_request_logic WHERE atmid IN (".$atmidarray.") order by id desc";
		}else{
			$onlinetestsql = "SELECT * FROM footage_request_logic WHERE atmid IN (".$atmidarray.") and status='".$status."' order by id desc";
		}
						  ...
						  if(mysqli_num_rows($sql)>0){
							  while($sql_result = mysqli_fetch_assoc($sql)){
								...
								if($sql_result['is_checked']==0){
									...
									$update_query = "UPDATE footage_request_logic SET footage_avail='".$footage_avail."',downlink='".$downloadlink."',is_checked=1 WHERE id='".$_id."'";
