...
$code = 201;
        ...
        $fileName = 'alert-type.zip';
		
		$con = OpenCon();
        $getimagesql = "SELECT File_loc,ATMCode FROM `ai_alerts` WHERE alerttype='Weapon' GROUP BY ATMCode ORDER BY id DESC"; 
        ...
        if(mysqli_num_rows($getimagesdata)>0){
            while($getimages_result = mysqli_fetch_assoc($getimagesdata)){
				...
				$file = "alert_type_images/" . $atm_code . '.png';
				$success = file_put_contents($file, $data);
				array_push($imagesdata_array,$file);
            }
        }
        if(count($imagesdata_array)>0){
			if(file_exists('alert-type.zip')){
				unlink('alert-type.zip');
           ...
           /* $result = create_zip($files_to_zip,$fileName);
			...
			// Get real path for our folder
				$rootPath = realpath('alert_type_images');
