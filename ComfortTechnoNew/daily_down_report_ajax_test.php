                    ...
                    <table id="order-listing" class="table">
                      <thead >
                        <tr>
							...
							<th>ATM-ID</th><th>Circle</th><th>ZO</th>
							<th>State</th>
							<th>Location</th>
							<th>Site Type</th>
							<th>System Down From</th>
							<th>System Down Up To</th>
							<th>Total Downtime of a Day</th>
							<th>Site Up Within TAT</th>
							<th>Reason</th>
							<th>Whether Bank has advised to stop Surveillance</th>
							<th>Whether power supply inadequate for battery charging</th>
							<th>Operator Comments</th>
							<th>Total Downtime In Current Month in Hours</th>						
                        </tr>
                      </thead>
                      <tbody>
					    <?php $yesterday = date('Y-m-d',strtotime("-1 days"));
						      $today = date('Y-m-d');
							 $check_cnt = 0;$check_cnt_not = 0;
						 ...
						 if(mysqli_num_rows($sql)){
							while($sql_result = mysqli_fetch_assoc($sql)){
								...
								if($_view == 1){
									...
									$dwn_qry_sql = "SELECT * FROM `newdowntime_report` WHERE SN='".$sn."' AND month_date>='".$start_date."' AND month_date<='".$end_date."'";
									//echo $net_qry_sql;die;
									$dwn_sql_res = mysqli_query($con,$dwn_qry_sql);
									
									if(mysqli_num_rows($dwn_sql_res)>0){
										$check_cnt = $check_cnt + 1;
									/*	while($dwn_data = mysqli_fetch_assoc($dwn_sql_res)){
											$dwn_date = $dwn_data['month_date'];
											$check_cnt = $check_cnt + 1;
											$system_down_from = $dwn_data['system_down_from'];
											$system_down_up_to = $dwn_data['system_down_up_to'];
											$total_downtime_day = $dwn_data['total_downtime_day'];
											$site_up_with_tat = $dwn_data['site_up_with_tat'];
											$reason = $dwn_data['reason'];
											$bank_advised_stop_surveillance = $dwn_data['bank_advised_stop_surveillance'];
											$power_supply_inadequate_battery = $dwn_data['power_supply_inadequate_battery'];
											$operator_comments = $dwn_data['operator_comments'];
											$total_downtime_current_month = $dwn_data['total_downtime_current_month'];
											$count++;
									...
									?>
									   <tr>
										   ...
										   <td><?php echo $dwn_date;?>
										   <td><?php echo $atm_id;?></td><td><?php echo $_circleName;?></td><td><?php echo $_Zone;?></td>
										   <td><?php echo $sql_result['State'];?></td>
										   <td><?php echo $site_address;?></td>
						                   <td><?php echo $site_type;?></td><td><?php echo $system_down_from;?></td>
										   <td><?php echo $system_down_up_to;?></td><td><?php echo $total_downtime_day;?></td>
										   <td><?php echo $site_up_with_tat;?></td><td><?php echo $reason;?></td>
										   <td><?php echo $bank_advised_stop_surveillance;?></td><td><?php echo $power_supply_inadequate_battery;?></td>
										   <td><?php echo $operator_comments;?></td><td><?php echo $total_downtime_current_month;?></td>
										</tr>
								
								<?php   }  */
									   }else{
										   echo "SN : ".$sn."_";
										   $check_cnt_not = $check_cnt_not + 1;
									   }
									  }
								   }
								 }
								  CloseCon($con);
								  echo $check_cnt.'_'.$check_cnt_not;
								?>
                      </tbody>
                    </table>
                  </div>
