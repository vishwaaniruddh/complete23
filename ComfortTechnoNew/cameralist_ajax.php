...
if(count($_circle_name_array)>0){
			...
			$query1 = "SELECT s.*,c.current_status,c.id AS cl_id,ticket_id,c.created_at  FROM `sites` s join call_log_camera_alerts c ON c.ATMID=s.ATMID WHERE s.ATMID IN (".$circleatmidarray.") AND s.live='Y'";
			
	}else{ 
		$query1 = "SELECT s.*,c.current_status,c.id AS cl_id,ticket_id,c.created_at  FROM `sites` s join call_log_camera_alerts c ON c.ATMID=s.ATMID WHERE s.live='Y'"; 
	} 
