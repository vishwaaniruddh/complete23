...
.tooltip:hover .tooltiptext {
  visibility: visible;
         ...
         <?php include('top-navbar.php');
				...
				$getimagesql = "SELECT * FROM `footage_details_available_start_zip` WHERE footage_status=1 AND month='".$mon."' AND dt='".$dt."' AND footage_type='Rejected'"; 
				//echo $getimagesql;
						    ...
						    <div class="card-body">
							    <div class="row">
									<div class="col-lg-4 col-xl-4">
										...
										<button class="btn btn-primary" onclick="start_Zip()">Start Merge</button>
									</div>
									<div class="col-lg-4 col-xl-4">
										<input type="hidden" id="final_zip_date" value="<?php echo $dt;?>">
										<button class="btn btn-primary" id="final_zip" style="display:none;" onclick="start_finalZip()">Final Zip</button>
									</div>
								</div>
							</div>
							<div class="card-body">
								 ...
								 <div class="row">
								    <?php if(mysqli_num_rows($getimagesdata)>0){
									          while($foot_data = mysqli_fetch_assoc($getimagesdata)){
													 ...
													 $file = urlencode($file);
													 $path = "E:/Footage_Upload/Rejected/$file";
       ...
       <script>
		...
		function checkStartZipProcess(){
			...
			var footage_type = 'Rejected';
			$.ajax({
						url: "bulkfootageinsert/check_start_zip_processing_1.php", 
						type: "POST",
						...
						success: (function (result) { debugger;
							...
							if(result>0){
								//alert("Zip started and it is in process so wait..."+ txt);
								checkStartZipProcess();
							}else{
								$("#load").hide(); 
								alert("Merge Completed now you can start zip");
								$("#final_zip").css("display","block");
								//location.reload();
		...
		function checkFinalZipProcess(){
			var dt = $('#start_zip_date').val();
			var footage_type = 'Rejected';
			$.ajax({
						url: "bulkfootageinsert/check_start_zip_processing_2.php", 
						type: "POST",
						data: {dt:dt,footage_type:footage_type},
						success: (function (result) { debugger;
							//$("#load").hide(); 
		...
		function start_Zip(){ 
				...
				$("#load").show();
				$.ajax({
							url: "bulkfootageinsert/footage_start_zip_rejected_first_2.php", 
							type: "POST",
							data: {dt:dt},
							success: (function (result) { debugger;
								//$("#load").hide(); 
		...
		function start_finalZip(){ 
				var dt = $('#start_zip_date').val();
				$("#load").show();
				$.ajax({
							url: "bulkfootageinsert/footage_start_zip_rejected_first_3.php", 
							type: "POST",
							data: {dt:dt},
							success: (function (result) { debugger;
								//$("#load").hide(); 
