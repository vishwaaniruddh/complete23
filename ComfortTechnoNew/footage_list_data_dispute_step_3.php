...
.tooltip:hover .tooltiptext {
  visibility: visible;
         ...
         <?php include('top-navbar.php');
				...
				$getimagesql = "SELECT * FROM `footage_details_available_start_zip` WHERE footage_status=1 AND month='".$mon."' AND dt='".$dt."' AND footage_type='Dispute'"; 
				//echo $getimagesql;
								 ...
								 <div class="row">
								    <?php if(mysqli_num_rows($getimagesdata)>0){
									          while($foot_data = mysqli_fetch_assoc($getimagesdata)){
													 ...
													 $file = urlencode($file);
													 $path = "D:/Footage_Upload/".$yr."/Dispute/$file";
									...
									<div class="col-lg-3 col-xl-2" style="border: 1px solid;padding:8px;">
                                        ...
                                        <div class="row">										
											...
											<div class="col-md-6 col-xl-6 ">
											    ...
											    <button class="btn btn-info" id="<?php echo "copy_".$_atmid_val;?>" style="display:none;" onclick="myFunction('<?php echo 'http://103.141.218.26:8080/Hitachi/download_footage.php?path='.$path;?>')" title="Click To Copy Download URL"><i class="fa fa-copy"></i></button>
											</div>
                                        </div>	
                                        <?php } }else{ ?>	
                                         <div class="row">										
       ...
       <script>
		...
		function start_Zip(){ 
				...
				$("#load").show();
				$.ajax({
							url: "bulkfootageinsert/footage_start_zip_dispute_first_1.php", 
							type: "POST",
							data: {dt:dt,yr:yr},
							success: (function (result) { debugger;
								//$("#load").hide(); 
