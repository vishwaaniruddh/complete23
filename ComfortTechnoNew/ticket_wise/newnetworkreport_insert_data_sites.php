...
//$month = 6;
$bank = "PNB"; 
$sql_qry = "select ATMID,SN,SiteAddress,DVRIP,PanelIP,RouterIp,ATMID_2,ATMID_3,live_date,Zone,State from sites where Customer='" . $client . "' and Bank='" . $bank . "' and live='Y' AND SN NOT IN (SELECT SN from `newnetwork_report` WHERE month_date='".$mon_date."')";
//echo $sql_qry;die;
...
if(mysqli_num_rows($sql)){
	...
	while($sql_result = mysqli_fetch_assoc($sql)){
			...
			if($_view == 1){
				...
				$set_sql = "INSERT INTO `newnetwork_report`( `SN`,`month_date`, `month`, `year`, `uptime`, `downtime`,  `created_at` ) VALUES ('" . $SN . "','" . $mon_date . "','" . $month . "','" . $year . "','0','0','" . $created_at . "') ";	
			//	echo $set_sql;die;
