									...
									?>
									   <tr>
										   ...
										   <?php //for($i=0;$i<count($list1);$i++){ 
													...
													if($total_net_his>0){
													   	while($net_his_sql_result = mysqli_fetch_assoc($net_sql_res)){
															...
															$atlastmonthdate = $_mon_dt;
															if($site_live_date<=$_mon_dt){
																$is_lies = 1;
																if($_mon_dt<=$today){
										   ...
										   <td><?php echo $total_time;?></td>
													<?php } 
													       // echo 'tot : '.$tot_cnt.' tot_days : '.$total_days;
													    if($tot_cnt<$total_days){
															$newtot = $tot_cnt + 1;
															for($j=$newtot;$j<=$total_days;$j++){ 
															   // echo '$atlastmonthdate : '.$atlastmonthdate;
															    if($atlastmonthdate==$today){ ?>
																<td><?php echo '0';?></td>	
