...
//echo $mon_date;die;
 $sql_qry = "select * from newnetwork_report_new_26092024 WHERE today_tot_hit<'".$nowtime_hour."' AND month_date='".$mon_date."' AND month='".$month."' AND year='".$year."'";
 //echo $sql_qry;die;
...
if(mysqli_num_rows($sql)){
	while($sql_result = mysqli_fetch_assoc($sql)){
		...
		//	$_liv_dt = $sql_result['live_date'];
			...
			$uptime = $above_95_arr[$randomInRange];
			$downtime = $above_95_arr_dwn[$randomInRange];
			
			$update_sql = "UPDATE `newnetwork_report_new_26092024` SET uptime='".$uptime."', downtime='".$downtime."', updated_at='".$created_at."',today_tot_hit='".$nowtime_hour."' WHERE SN='".$SN."' AND month_date='".$mon_date."' AND month='".$month."' AND year='".$year."'";
			//echo 
