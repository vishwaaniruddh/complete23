...
.tooltip:hover .tooltiptext {
  visibility: visible;
         ...
         <?php include('top-navbar.php');
				...
				$getimagesql = "SELECT * FROM `footage_details_available_start_zip_test` WHERE footage_status=1 AND month='".$mon."' AND dt='".$dt."' AND footage_type='24Hrs'"; 
				//echo $getimagesql;
								 ...
								 <div class="row">
								    <?php if(mysqli_num_rows($getimagesdata)>0){
									          while($foot_data = mysqli_fetch_assoc($getimagesdata)){
													 ...
													 $file = urlencode($file);
													 $path = "E:/Footage_Upload_Test/24Hrs/$file";
       ...
       <script>
		...
		function checkStartZipProcess(){
			...
			$.ajax({
						url: "bulkfootageinsert/check_start_zip_processing_test.php", 
						type: "POST",
		...
		function start_Zip(){ 
				...
				$("#load").show();
				$.ajax({
							url: "bulkfootageinsert/footage_start_zip_24hrs_first_1_test.php", 
							type: "POST",
							data: {dt:dt},
							success: (function (result) { debugger;
								//$("#load").hide(); 
		...
		function get_Detail(atmid,atm){ 
				...
				$("#load").show();
				$.ajax({
							url: "footage_atm_list_zip_24hrs_test.php", 
							type: "POST",
