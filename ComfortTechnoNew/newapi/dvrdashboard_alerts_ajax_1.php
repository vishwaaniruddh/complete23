...
/*	echo '<pre>';print_r($res_data);echo '</pre>';
	...
	echo '<pre>';print_r($pielabeldata_array);echo '</pre>';die; */
	$_res_data = array();
	if($atmid!=''){ 
		...
		if(mysqli_num_rows($aisql)>0){
			...
			while($aisql_result = mysqli_fetch_assoc($aisql)){
			   ...
			   $single_aisql = mysqli_query($con,"select alerttype,File_loc,createtime,ATMCode from ai_alerts where ATMCode like '%".$atmid."%' AND alerttype like '%".$alert_type."%' AND receivedtime >= SUBDATE( NOW(), INTERVAL 24 HOUR)");
			   $total_ai_alert = mysqli_num_rows($single_aisql);
			   if($total_ai_alert){
			       $cnt = $total_ai_alert;
			...
			while($aisql_result = mysqli_fetch_assoc($aisql)){
			   ...
			   $atm_code = ltrim($aisql_result['ATMCode']);
				...
				if (in_array($atm_code, $atmCodearray)){
				   ...
				   $single_aisql = mysqli_query($con,"select alerttype,File_loc,createtime,ATMCode from ai_alerts where ATMCode like '%".$atm_code."%' AND alerttype like '%".$alert_type."%' AND receivedtime >= SUBDATE( NOW(), INTERVAL 24 HOUR)");
				   $total_ai_alert = mysqli_num_rows($single_aisql);
				   if($total_ai_alert){
					   $cnt = $total_ai_alert;
					/* while($ai_sql_result = mysqli_fetch_assoc($single_aisql)){
