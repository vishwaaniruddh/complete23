...
if($atmid!=''){
	...
	if($bank!=''){
		if($circle!=''){
			...
			if($status=='All'){
				$sql = mysqli_query($con,"select * from sites where Customer='".$client."' and Bank='".$bank."' and ATMID IN (".$circleatmidarray.")"); 
			}
			if($status=='Online'){
			...
			if($_circle_ids!=''){
				if($status=='All'){
					$sql = mysqli_query($con,"select * from sites where Customer='".$client."' and Bank='".$bank."' and ATMID IN (".$_circle_name_array1.")"); 
				}
				if($status=='Online'){
				...
				if($status=='All'){
					$sql = mysqli_query($con,"select * from sites where Customer='".$client."' and Bank='".$bank."'"); 
				}
				if($status=='Online'){
		...
		if($client=='All'){
			...
			if($status=='Online'){
				//$sql = mysqli_query($con,"select * from sites where Bank IN (".$_bank_name.") and live='Y'");
				$sql = mysqli_query($con,"select * from sites where live='Y'");
			}
			if($status=='Offline'){
			//	$sql = mysqli_query($con,"select * from sites where Bank IN (".$_bank_name.") and live!='Y'");
...
//echo $sql_qry;die;
 ...
 $yesterday = date('Y-m-d',strtotime("-1 days"));
			  ...
			  $array = array(['count'=>$count,'res_data'=>$_data,'bank_name'=>$_bank_name]);
					CloseCon($con);
