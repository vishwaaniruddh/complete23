...
function getClients_ios($userid){
    global $con;
	$usersql = mysqli_query($con,"select cust_id from loginusers where id='".$userid."'");
	$userdata = mysqli_fetch_assoc($usersql);
	$_client_ids = $userdata['cust_id'];
	...
	$html = [];
	while($sql_result = mysqli_fetch_assoc($sql)){
		$value['label'] = $sql_result['Customer'];
		$value['value'] = $sql_result['Customer'];
		array_push($html,$value);
	}
	return $html;
...
function getBanksByClientID_ios($client,$userid){
    global $con;
	$usersql = mysqli_query($con,"select cust_id,bank_id from loginusers where id='".$userid."'");
	$userdata = mysqli_fetch_assoc($usersql);
	$_bank_ids = $userdata['bank_id'];
	...
	$html = [];
	while($sql_result = mysqli_fetch_assoc($sql)){
		$value['label'] = $sql_result['Bank']; 
		$value['value'] = $sql_result['Bank']; 
		$bankvalue = $sql_result['Bank']; 
		$cust_bank = $client."_".$bankvalue;
		if(in_array($cust_bank,$banks)){
		array_push($html,$value);
		}
...
function getAtmidCircleList($bank,$circle){
	...
	$sql = mysqli_query($con,"SELECT ATMID FROM sites WHERE live='Y' AND ATMID IN (select ATMID from site_circle where Bank='".$bank."' AND Circle='".$circle."')");
