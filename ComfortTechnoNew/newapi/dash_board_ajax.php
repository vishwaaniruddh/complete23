		...
		while($sql_result = mysqli_fetch_assoc($sql)){
		   ...
		   // $ai_alert_sql = mysqli_query($con,"select File_loc,createtime,alerttype,sendip from ai_alerts_alive where createtime >= SUBDATE( NOW(), INTERVAL 24 HOUR) AND ATMCode like '%".$atm_id."%' ORDER BY id desc LIMIT 1"); 
			...
			//	$src = 'data: jpeg;base64,'.$file; 
			 ...
			 $data_arr = [];
			        $data_arr['id'] = $ticket_id; 
					$data_arr['atmid'] = $sql_result['ATMID'];
					$data_arr['datetime'] = $createdatetime;
					$data_arr['src'] = $src;
					$data_arr['alert_name'] = $AlertType;
...
$array = array(['code'=>$code,'res_data'=>$_data,'camera_working_count'=>$camera_working_count,'camera_notworking_count'=>$camera_notworking_count,'total_site'=>$total_site]);
