			...
			for($j=0;$j<count($newfile_list);$j++){
				...
				if(count($split_atm_image)==5){
					...
					if($check_atmid!='mp4' && $check_atmid!='dav' && $check_atmid!='avi'){
						$check_footage = "SELECT * FROM `footage_details_available_start_zip_test` WHERE month='".$mon."' AND dt='".$dt."' AND atmid='".$_atmid_imageval."' AND footage_type='".$footage_type."'";
						...
						if(mysqli_num_rows($check_footage_res)==0){
							...
							$local_file_dir = "E:/Footage_Upload_Test/24Hrs/".$_temp_folder_path;
							$local_path = "24Hrs/".$_temp_folder_path;
							...
							if(count($finalfile_list)>0){ 
								...
								if(count($imagesdata_array)>0){
									...
									$remarks = "";
									$set_sql = "INSERT INTO `footage_details_available_start_zip_test`( `month`, `dt`,`atmid`, `created_at`, `updated_at`, `created_by`, `footage_type`, `ftp_server_path`, `local_path`, `remarks`) VALUES ('".$mon."','".$dt."','".$_atmid_imageval."','".$current_datetime."','".$current_datetime."','24','".$footage_type."','".$ftp_server_path."','".$local_path."','".$remarks."') ";
