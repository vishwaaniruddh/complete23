...
$bank = "PNB"; 
$sql_qry = "SELECT * FROM `newnetwork_report` WHERE month_date='".$mon_date."' AND SN NOT IN (
...
if(mysqli_num_rows($sql)){
	while($sql_result = mysqli_fetch_assoc($sql)){
		...
		$id = $sql_result['id'];
		$set_sql = "UPDATE `newnetwork_report` SET `uptime`='".$nowtime_hour."', `downtime`='0',`is_update`='1',`today_tot_hit`='".$nowtime_hour."' where id='" . $id . "'";
	    $set_result = mysqli_query($con, $set_sql);
	}
}

$sql_qry = "SELECT * FROM `newnetwork_report` WHERE month_date='".$mon_date."' AND SN IN (
'3284','3334','3533','3378','3510','3518','3639','3844','3865','3950','3989','4029','4112','4200','4219','4549','10060','4538','4635','4764','4968','5051','5081','10125','5636','5204','6255',
'3658'
)";
...
if(mysqli_num_rows($sql)){
	while($sql_result = mysqli_fetch_assoc($sql)){
		...
		$id = $sql_result['id'];
		$_up = $sql_result['uptime'];
		$_down = $sql_result['downtime'];
		$now_up = $_up + 1;
		$set_sql = "UPDATE `newnetwork_report` SET `uptime`='".$now_up."', `is_update`='1',`today_tot_hit`='".$nowtime_hour."' where id='" . $id . "'";
	    $set_result = mysqli_query($con, $set_sql);
	}
}
