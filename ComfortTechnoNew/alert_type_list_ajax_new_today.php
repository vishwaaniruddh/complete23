...
//$end = $_GET['end'];
$status = 0;
$_s_q_l = "select * from alert_type_month_date where month_date='".$today."' AND today_tot_hit<'".$nowtime_hour."' limit 1";
//echo $_s_q_l;die;
$alert_type_sql = mysqli_query($con,"select * from alert_type_month_date where month_date='".$today."' AND today_tot_hit<'".$nowtime_hour."' limit 1");
if(mysqli_num_rows($alert_type_sql)>0){
			...
			if($zone_no_data!=''){
				if(strlen($_site_panel_id)==6){
					...
					*/
						...
						$zone_query = "SELECT * FROM alerts WHERE zone IN (".$zone_no_data.") AND CAST(createtime AS DATE)>='".$start."' AND CAST(createtime AS DATE)<='".$end."' AND panelid ='".$_site_panel_id."' AND alerttype IS NOT NULL AND id NOT IN (SELECT ticket_id FROM alerts_home_dashboard WHERE zone IN (".$zone_no_data.") AND CAST(createtime AS DATE)>='".$start."' AND CAST(createtime AS DATE)<='".$end."' AND panelid ='".$_site_panel_id."' AND alerttype IS NOT NULL)";
						//echo $zone_query.'</br>';die;
						$check_data = mysqli_query($con, $zone_query);
						
						if(mysqli_num_rows($check_data)>0){
							//echo $zone_query.'</br>';die;
							
							//$zone_alert_data = mysqli_fetch_assoc($check_data);
	...
	if($total_inserted>0){
		$update_qry = mysqli_query($con,"UPDATE alert_type_month_date SET today_tot_hit='".$nowtime_hour."' WHERE id='".$alert_type_id."'");
