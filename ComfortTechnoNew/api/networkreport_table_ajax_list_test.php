...
if($atmid!=''){
	...
	if($bank!=''){
	  $sql = mysqli_query($con,"select ATMID,SN,SiteAddress,DVRIP,PanelIP,RouterIp from sites where Customer='".$client."' and Bank='".$bank."' and live='Y'");
		...
		if($client=='All'){
		$sql = mysqli_query($con,"select ATMID,SN,SiteAddress,DVRIP,PanelIP,RouterIp from sites where Bank IN (".$_bank_name.") and live='Y'");	
		}else{
							...
							while($sql_result = mysqli_fetch_assoc($sql)){
								...
								if(mysqli_num_rows($aisql)>0){
									...
									//	$networksql = mysqli_query($con,"select device,status from network_history where site_id ='".$sn."'"); 	
										...
										/*$dvrnetworksql = mysqli_query($con,"select count(id) from network_history where site_id ='".$sn."' AND status='OK' AND device='D'");
                                        if(mysqli_num_rows($dvrnetworksql)>0){
											$dvrnetwork_result = mysqli_fetch_row($dvrnetworksql);
											$dvr_status_online = $dvrnetwork_result[0];
										} 		*/								
