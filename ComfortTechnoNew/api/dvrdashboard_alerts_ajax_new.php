...
if(mysqli_num_rows($sql)){
	while($sitesql_result = mysqli_fetch_assoc($sql)){
		...
		$alerts_sql = "SELECT id,status,sendtoclient FROM alerts WHERE panelid ='".$panelid."' AND sendtoclient='S' AND CAST(receivedtime AS DATE)>='".$query_date."'"; 
	...
	for($i=1;$i<=$end_day;$i++){
		...
		while($_sitesql_result = mysqli_fetch_assoc($_sql)){
			...
			$_alerts_sql = "SELECT id,status,sendtoclient FROM alerts WHERE panelid ='".$_panelid."' AND sendtoclient='S' AND CAST(createtime AS DATE)='".$create_date."'";
