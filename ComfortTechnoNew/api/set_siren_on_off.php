<?php include('db_connection.php'); 
 ...
 $created_at = date('Y-m-d H:i:s');
  ...
  function headersToArray( $str )
	...
	function setSiren($url,$access_token,$org_id,$mac_id,$hooter_status){
		$curl = curl_init();
...
$response = setSiren($url,$access_token,$org_id,$mac_id,$hooter_status);
...
if($check['statusCode']==401){
	...
	if($new_access_token!=''){
		...
		$response = setSiren($url,$new_access_token,$org_id,$mac_id,$hooter_status);
        $check = json_decode($response,true);
	...
	mysqli_query($con,"insert into hooter_siren_command_status (mac_id,current_status,created_at) values ('".$mac_id."','".$hooter_status."','".$command_type."','".$created_at."') ");
	$array = array(['Code'=>200,'res_data'=>$check]);
	CloseCon($con);
    echo json_encode($array);
}
