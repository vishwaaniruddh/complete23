<?php include('db_connection.php'); 
 date_default_timezone_set('Asia/Kolkata');
  ...
  function headersToArray( $str )
	...
	function getDeviceList($url,$access_token){
...
$url = 'https://eazyinfra.utopiatech.in:4510/panelmeterloglist?org_id='.$org_id.'&mac_id='.$mac_id;
...
$newurl = 'https://eazyinfra.utopiatech.in:4510/panel/filterlist?org_id='.$org_id.'&group_id=0&state=All';
$device_response = getDeviceList($newurl,$access_token);
	$_check = json_decode($device_response,true);
	$_newdata = [];
if($_check['statusCode']==200){
	$gwlist = $_check['result']['gwlist'];
	$finalarray = array();
...
if(count($_newdata)>0){
	$array = array(['Code'=>200,'device_data'=>$_newdata,'res_data'=>$response]);
}else{
