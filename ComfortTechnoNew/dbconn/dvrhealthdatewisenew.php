		...
		while($sitesql_result = mysqli_fetch_assoc($sql)){
			...
			$testsql = "SELECT status FROM network_history WHERE site_id ='".$SN."' AND CAST(rectime AS DATE)>='".$start."' 
						...
						$totaldvrconnect = mysqli_num_rows($dvrhis_query);   
							...
							$checkmonthsql = mysqli_query($con,"select id from dvr_health_site_datewise where SN='".$SN."' and month_date='".$month."'");
							if (!$checkmonthsql || mysqli_num_rows($checkmonthsql) == 0){
							//if(mysqli_num_rows($checkmonthsql)==0){ 					
								$created_at = date('Y-m-d H:i:s');
								$set_sql = "INSERT INTO `dvr_health_site_datewise`( `SN`, `month_date`, `online_status_count`, `offline_status_count`, `online_percent`, `offline_percent`, `site_address`, `created_at`) VALUES ('".$SN."','".$month_date."','".$dvr_online_count."','".$dvr_offline_count."','".$total_online_percent."','".$total_offline_percent."','".$siteaddress."','".$created_at."') ";
								$set_result = mysqli_query($con,$set_sql); 
							}else{
								$monthsqlres = mysqli_fetch_assoc($checkmonthsql);
								$id = $monthsqlres['id'];
								$updatesql= " UPDATE `dvr_health_site_datewise` SET `online_status_count`='".$dvr_online_count."',`offline_status_count`='".$dvr_offline_count."',`online_percent`='".$total_online_percent."',`offline_percent`='".$total_offline_percent."' where `id`='".$id."'";
