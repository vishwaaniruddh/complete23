...
$banks = explode(",",$_SESSION['bankname']);
	...
	if($atmid!=''){
		$query = "SELECT ATMID from dvronline WHERE ATMID='".$atmid."' and Status='Y'";
		...
		if($bank!=''){
			if($circle!=''){
					...
					$query = "SELECT ATMID from dvronline WHERE customer='".$client."' and Bank='".$bank."' and ATMID IN (".$circleatmidarray.") and Status='Y'";
			}else{ 
			    $query = "SELECT ATMID from dvronline WHERE customer='".$client."' and Bank='".$bank."' and Status='Y'";
					
			} 
		 
		}else{ 
		    $query = "SELECT ATMID from dvronline WHERE customer='".$client."' and Bank IN (".$_bank_name.") and Status='Y'";
	...
	$ftp_conn = OpenFTPCon();
	//$ftp_conn = OpenComfortFTPLocalCon();
	$ftp_pasv = ftp_pasv($ftp_conn,true);
	 
	//$file_list = ftp_nlist($ftp_conn, "./CLOUD");
	$file_list = ftp_nlist($ftp_conn, "./CLOUD");
	//$file_list = ftp_nlist($ftp_conn, "./CLOUD/MPZ08001/2023-07-14/001/dav");
	...
	if(count($all_atm)>0){
		for($i=0;$i<count($all_atm);$i++){
			...
			if($all_atm[$i]!='desktop.ini'){
				...
				$path = "./CLOUD/".$all_atm[$i];  //echo $path;die;
				...
				if(count($atmfile_list_share)>0){
					...
					for($z=0;$z<count($atmfile_list_share);$z++){
						if($atmfile_list_share[$z]!='desktop.ini' && $atmfile_list_share[$z]!='DVRWorkDirectory'){
						    $filepath_explode = explode('/',$atmfile_list_share[$z]);
						    
						    if(count($filepath_explode)==4){
								if($filepath_explode[3]!='desktop.ini' && $filepath_explode[3]!='DVRWorkDirectory'){
								//	echo '<pre>';print_r($filepath_explode);echo '</pre>';
								   $date_explode = explode('-',$filepath_explode[3]);
					...
					// echo '<pre>';print_r($year_arr);echo '</pre>';die; 
					//die;
					if(count($year_arr)>0){
					...
					//echo '<pre>';print_r($year_arr);echo '</pre>'; echo '<pre>';print_r($month_arr);echo '</pre>';echo '<pre>';print_r($date_arr);echo '</pre>';die; 
					if(count($year_arr)>0 && count($month_arr) && count($date_arr)>0){
						...
						if(count($mainfile_list_share)>0){
							for($z=0;$z<count($mainfile_list_share);$z++){
								if($mainfile_list_share[$z]!='desktop.ini' && $mainfile_list_share[$z]!='DVRWorkDirectory'){
									$folderfilepath_explode = explode('/',$mainfile_list_share[$z]);
									if(count($folderfilepath_explode)==7){
										if($folderfilepath_explode[6]!='desktop.ini' && $folderfilepath_explode[6]!='DVRWorkDirectory'){
									        array_push($last_folder_arr,$folderfilepath_explode[6]);
										}
									}
								}
							}
