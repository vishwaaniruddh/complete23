...
function lastcommunicationdiff($datetime1,$datetime2){
	...
	//$net_qry = "SELECT * from network_report_list WHERE last_action_date='".$today."' AND today_tot_hit<'13' limit 700";
	$sql_qry = "SELECT * from network_report_list WHERE last_action_date='".$today."' AND today_tot_hit<'13' limit 700";
				...
				if(mysqli_num_rows($aisql)>0){
						...
						if($check_network_report_list_cnt==0){
							...
							*/
							...
							if($will_update==1){
								$updatesql = "update network_report_list SET SN='".$sn."',ATMID='".$atm_id."',router_status='".$router_status."',router_lastcommunication='".$routerlast_communication."',router_online_count='".$router_status_online."',router_tot_count='".$router_status_total."',router_online_percent='".$routeronlinepercent."',
											  dvr_status='".$dvr_status."',dvr_lastcommunication='".$dvrlast_communication."',dvr_online_count='".$dvr_status_online."',dvr_tot_count='".$dvr_status_total."',dvr_online_percent='".$dvronlinepercent."',
											  panel_status='".$panel_status."',panel_lastcommunication='".$panellast_communication."',panel_online_count='".$panel_status_online."',panel_tot_count='".$panel_status_total."',panel_online_percent='".$panelonlinepercent."',last_action_date='".$today."',`is_update`='1', `today_tot_hit`='".$nowtime_hour."' where id='".$tb_id."'";
								//echo $updatesql;die;
