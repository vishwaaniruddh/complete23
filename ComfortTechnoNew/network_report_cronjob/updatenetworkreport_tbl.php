...
function lastcommunicationdiff($datetime1,$datetime2){
	...
	$yesterday = date('Y-m-d',strtotime("-1 days"));
	$chk_net_qry = mysqli_query($con,"SELECT * from network_report WHERE CAST(router AS DATE)='2023-07-22' AND SN IN (SELECT SN FROM sites WHERE Customer='Hitachi' AND Bank='PNB' AND live='Y')");
	if(mysqli_num_rows($chk_net_qry)>0){
		while($net_data = mysqli_fetch_assoc($chk_net_qry)){
			$net_SN = $net_data['SN'];
			$net_router = $net_data['router'];
			$net_router_td = explode(" ",$net_router);
			$net_dvr = $net_data['dvr'];
			$net_dvr_td = explode(" ",$net_dvr);
			$net_panel = $net_data['panel'];
			$net_panel_td = explode(" ",$net_panel);
			$new_router = $today_date." ".
