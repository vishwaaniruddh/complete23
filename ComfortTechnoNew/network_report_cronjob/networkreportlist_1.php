...
function lastcommunicationdiff($datetime1,$datetime2){
	...
	/*
	$net_qry = "SELECT * from network_report_list WHERE last_action_date='".$today."' AND today_tot_hit=0 limit 700";
    $sql_qry = "select s.live,s.Customer,s.Bank,s.ATMID,s.SN,s.SiteAddress,s.DVRIP,s.PanelIP,s.RouterIp,s.ATMID_2
		...
		*/		
	$sql_qry = "SELECT * from network_report_list WHERE last_action_date='".$today."' AND today_tot_hit=12 limit 700";			
  //  echo $sql_qry;die;
				...
				if(mysqli_num_rows($aisql)>0){
						...
						if($check_network_report_list_cnt==0){
							...
							*/
							...
							if($will_update==1){
								$updatesql = "update network_report_list SET router_status='".$router_status."',router_lastcommunication='".$routerlast_communication."',router_online_count='".$router_status_online."',router_tot_count='".$router_status_total."',router_online_percent='".$routeronlinepercent."',
