								...
								if($_view == 1){
									...
									if(mysqli_num_rows($aisql)>0){
										...
										if(!is_null($routerlast_communication)){
											$router_status_hr = lastcommunicationdiff($datetime1,$routerlast_communication);
											if($router_status_hr>0){
												$router_status = 1;
											}else{
												$router_status = 0;
											}
											...
											if(!is_null($dvrlast_communication)){
												$dvr_status_hr = lastcommunicationdiff($datetime1,$dvrlast_communication);
												if($dvr_status_hr>0){
													$dvr_status = 1;
												}else{
													$dvr_status = 0;
											...
											if(!is_null($panellast_communication)){
												$panel_status_hr = lastcommunicationdiff($datetime1,$panellast_communication);
												if($panel_status_hr>0){
													$panel_status = 1;
												}else{
													$panel_status = 0;
												}
										...
										$check_network_report_list = mysqli_query($con,"select * from network_report_list where SN='".$sn."' AND last_action_date='".$today."'");
										$check_network_report_list_cnt = 1;
										if(mysqli_num_rows($check_network_report_list)==0){ 
											$check_network_report_list_cnt = 0;
											$networksql = mysqli_query($con,"select device,status,rectime from network_history where site_id ='".$sn."' AND CAST(rectime AS DATE)<='".$today."'"); 
											$is_networksql = 1;   
										}else{
											...
											if($tb_last_action_date==$today){
											   //echo mysqli_num_rows($networksql);
											  // $will_update = 1;
											//}else{ 
											   $netrouterlast_communication = $net_rep_list['router_lastcommunication'];
											   $netdvr_status_hr = lastcommunicationdiff($datetime1,$netrouterlast_communication);
											   
											   $networksql = mysqli_query($con,"select device,status,rectime from network_history where site_id ='".$sn."' AND CAST(rectime AS DATE)='".$today."'"); 
											   if($netdvr_status_hr>3){
											     $will_update = 1;
											   }
											   $is_networksql = 1;
											}
										...
										$netdvrlast_communication = '';
                                        $netpanellast_communication = '';
                                        $netrouterlast_communication = '';
											...
											if(mysqli_num_rows($networksql)>0){
												while($network_result = mysqli_fetch_assoc($networksql)){
													if($network_result['device']=='D'){
														...
														$dvrlast_communication = $network_result['rectime'];
													}
													if($network_result['device']=='P'){
														...
														$panellast_communication = $network_result['rectime'];
													}
													if($network_result['device']=='R'){
														...
														$routerlast_communication = $network_result['rectime'];
													}
												}
											}
									    }
							        ...
							        if($check_network_report_list_cnt==0){
										...
										//$tb_id = $net_rep_list['id'];
										if($will_update==1){
											$updatesql = "update network_report_list SET SN='".$sn."',Customer='".$customer."',Bank='".$bank."',ATMID='".$atm_id."',SiteAddress='".$site_address."',router_status='".$router_status."',router_ip='".$routerip."',router_lastcommunication='".$routerlast_communication."',router_online_count='".$router_status_online."',router_tot_count='".$router_status_total."',router_online_percent='".$routeronlinepercent."',
														  ...
														  panel_status='".$panel_status."',panel_ip='".$panelip."',panel_lastcommunication='".$panellast_communication."',panel_online_count='".$panel_status_online."',panel_tot_count='".$panel_status_total."',panel_online_percent='".$panelonlinepercent."',last_action_date='".$today."',live='".$live."' where id='".$tb_id."'";
											//echo $updatesql;die;
