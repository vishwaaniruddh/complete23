	...
	if($bank!=''){
		if($circle!=''){
			...
			if(count($_circle_name_array)>0){
			   $sql = mysqli_query($con,"select ATMID,SN,SiteAddress,DVRIP,PanelIP,RouterIp,Bank,Customer from sites where Customer='".$client."' and Bank='".$bank."' and ATMID IN (".$_circle_name_array.") and live='Y'");
		   }else{  
		...
		if($_view == 1){
			...
			if(mysqli_num_rows($aisql)>0){
				...
				if($dvr_status>0){
					...
					$persitesql = mysqli_query($con,"select * from dvr_health where atmid='".$atm_id."'");
					if(mysqli_num_rows($persitesql)>0){
						$checksql_result = mysqli_fetch_assoc($persitesql);
						if(strtoupper($checksql_result['cam1'])=='WORKING'){
							$cam1_cnt = $cam1_cnt + 1;
						...
						$data_arr['lobby_cam'] = strtoupper($checksql_result['cam1']);
						$data_arr['backroom_cam'] = strtoupper($checksql_result['cam2']);
						$data_arr['outdoor_cam'] = strtoupper($checksql_result['cam3']);
						$data_arr['pinhole_cam'] = strtoupper($checksql_result['cam4']);
						$data_arr['last_comm'] = $checksql_result['last_communication'];
						$data_arr['cdate'] = $checksql_result['cdate'];
						$data_arr['capacity'] = $checksql_result['capacity'];
						$data_arr['freespace'] = $checksql_result['freespace'];
						$data_arr['recording_from'] = $checksql_result['recording_from'];
						$data_arr['recording_to'] = $checksql_result['recording_to'];
...
$array = array(['dvr_online_count'=>$dvr_online_count,'dvr_offline_count'=>$dvr_offline_count,'panel_online_count'=>$panel_online_count,'panel_offline_count'=>$panel_offline_count,
				 ...
				 'camera1'=>$cam1_cnt,'camera2'=>$cam2_cnt,'camera3'=>$cam3_cnt,'camera4'=>$cam4_cnt,'res_data'=>$_data]);
CloseCon($con);
