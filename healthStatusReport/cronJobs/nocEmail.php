...
if ($inbox) {
    ...
    if ($unseenMessages) {
        foreach ($unseenMessages as $messageNumber) {
            ...
            echo 'message_id' . $message_id = getHeaderValue($header, 'Message-ID');
            echo PHP_EOL;
            echo 'references' . $references = getHeaderValue($header, 'References');
            echo PHP_EOL;

            echo 'in_reply_' . $in_reply_to = getHeaderValue($header, 'In-Reply-To');
            echo PHP_EOL;
            ...
            if (isset($atmID) && !empty($atmID)) {
                ...
                $emailBody = $emailContent = str_replace('Content-Transfer-Encoding: quoted-printable', '', quoted_printable_decode($emailBody));
                ...
                // if (strpos($subject, $storedSubject) === false) {
                // return ; 
                // }
