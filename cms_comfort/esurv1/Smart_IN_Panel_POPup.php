                        ...
                        <div class="card-box">
                            ...
                            <div class="text-center mt-4 mb-4">
                                <div class="row">
                                    ...
                                    <?php  if($value=="ConnSmartIN" || $value=="NotConnSmartIN"){  ?>
                                    <div class="col-md-6 col-xl-3">
                                        <div class="card-box bg-danger widget-flat border-danger text-white" style="padding-bottom: 0px;">
                                            <i class="fi-delete" style="pointer-events: none; cursor: default;">Securico</i>
                                             <h>Smart-IN</h>
                                           <h3 class="m-b-10"><?php $Connectqry=mysqli_query($conn,"select count(*) as totalConnect from panel_health where status='0' and panelName ='SMART-IN' ");
                                            $fetchConnectQry=mysqli_fetch_array($Connectqry);
                                            
                                            $NotConnectqry=mysqli_query($conn,"select count(*) as totalNotConnect from panel_health where status='1' and panelName ='SMART-IN'");
                                            $fetchNotConnectQry=mysqli_fetch_array($NotConnectqry);
                                            
                                            echo $fetchConnectQry['totalConnect'] . "/" .  $fetchNotConnectQry['totalNotConnect'];
                                            ?></h3>
                                            <p class="text-uppercase m-b-5 font-13 font-600"  ><a href="Smart_IN_Panel_POPup.php?id=ConnSmartIN" style="color:white" target="_blank">Connected</a> / <a href="Smart_IN_Panel_POPup.php?id=NotConnSmartIN" style="color:white" target="_blank">Not Connected</a></p>
                                        </div>
                                    </div><?php } ?>
                                </div>
                            </div>
...
<div align="center" style="display:none"><input type="text" id="Atmid" name="Atmid" placeholder="Search Atm-ID"  /><input type="button" value="search" onClick="a('','50')" /></div>
     ...
     <td ><div >ATM-ID</div></td>
      ...
      <td ><div >Date_Time&nbsp; </div></td>
        ...
        <td><div style="" id="">Backroom Door Open Sensor</div></td>
        
        <td><div id="">Motion Sensor</div></td>
        <td><div id="">Glass Break Sensor</div></td>
        <td><div style="" id="">ATM 1 Vibration Sensor</div></td>
        ...
        <td><div id="">Hooter sense (Sounder Trouble)</div></td>
        <td><div id="">Inbuilt (Inverted Zone)</div></td>
        <td><div id="">Panel Tamper (Inverted Zone)</div></td>
        <td><div style="" id="">Smoke Detector I/P</div></td>
        <td><div style="" id="">ATM Backroom</div></td>
        <td><div id="">ATM Main Door Shutter Sensor</div></td>
        <td><div style="" id="">Cheque Drop Box Removal Sensor</div></td>
        <td><div style="" id="">Cheque Drop Box Door open Sensor</div></td>
        <td><div style="" id="">CCTV1 + CCTV2+ CCTV3 Removal Sensor</div></td>
      
        <td><div style="" id="">Speaker +MIC Removal Sensor</div></td>
        <td><div id="">Housekeeping Alert</div></td>
        <td><div style="" id="">AC 1 2 Removal Sensor</div></td>
        <td><div id="">ATM Lobby Temperature </div></td>
        <td><div style="" id="">PIN PAD REMOVAL SENSOR</div></td>
        <td><div id="">ATM HOOD DOOR SENSOR</div></td>
 ...
 if($value=="ConnSmartIN" ){
 $qrys=mysqli_query($conn,"select * from panel_health where status='0' and panelName='SMART-IN' ");
 }else if($value=="NotConnSmartIN"){
 $qrys=mysqli_query($conn,"select * from panel_health where status='1' and panelName='SMART-IN' ");
 }
