...
if($_SESSION['permission']!="" && $_SESSION['designation']=="2"){
   ...
   $abc_count.=" and atmid IN (select ATMID from axissite where CTS_LocalBranch IN($per) and live='Y')";
   $abc.=" and atmid IN (select ATMID from axissite where CTS_LocalBranch IN($per) and live='Y')";
}
