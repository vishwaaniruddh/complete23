...
$sqlappCount="select count(*) as total from dvr_health where  live='y'";
...
if($_SESSION['permission']!="" && $_SESSION['designation']=="2"){
   ...
   $sqlappCount.=" and atmid IN (select ATMID from sites where CTS_LocalBranch IN($per) and live='Y')";
   $abc.=" and atmid IN (select ATMID from sites where CTS_LocalBranch IN($per) and live='Y')";
}
if($_SESSION['designation']=="4"){
    
   $sqlappCount.=" and atmid IN (select ATMID from sites where Customer='Hitachi' and live='Y')";
...
$result = mysqli_query($conn, $sqlappCount);
...
?>
  <tr <?php if($row[2]==0 || $row[10]==1){ ?>style="background-color:#FF0000" <?php } ?> >
       ...
       <td align="center"><div id=""><?php if($row[2]==1)echo 'good'; else echo 'bad'; ?><?php if($row[2]!=1){ ?><a this data-toggle="modal" data-status="" data-id="<?php echo $SN; ?>" class="open-DetailDialog btn btn-danger" href="#myModalDetail">Details</a><?php }?></div></td>
...
$given_date = date('Y-m-d', strtotime($datetime));
