...
<!--                             <div class="text-center mt-4 mb-4">
                                <div class="row">
                                    ...
                                    <?php }else if($value=="error"||$value=="notexist"||$value=="smartFailed"||$value=="unformatted"||$value=="nodisk"||$value=="abnormal"){  ?>
                                    <div class="col-md-6 col-xl-3" >
                                        <div class="card-box bg-primary widget-flat border-primary text-white" style="padding-left: 4px;padding-bottom: 0px;">
											...
											$abNormalqry=mysqli_query($conn,"select count(*) as totalAbNormal from dvr_health where hdd='abnormal' ");
                                            $fetchabNormalQry=mysqli_fetch_array($abNormalqry);
                                            
                                            $noDiskqry=mysqli_query($conn,"select count(*) as totalNoDisk from dvr_health where hdd='No Disk' or hdd='No disk/idle' ");
                                            ...
                                            ?></h3>
                                            <p class="text-uppercase m-b-5 font-13 font-600">Error/NotExist/SmartFail/UnFormated</br>/AbNormal/No Disk</p>
                                        </div>
                                    ...
                                    </div><?php }else if($value=="Cam1" || $value=="Cam2" || $value=="Cam3"|| $value=="Cam4"){  ?>
                                    <div class="col-md-6 col-xl-3" >
                                        <div class="card-box bg-danger widget-flat border-danger text-white" style="padding-bottom: 0px;">
                                            ...
                                            $Notworkingqry4=mysqli_query($conn,"select count(*) as totalNotworking from dvr_health where cam4='not working'  ");
                                            ...
                                            <p class="text-uppercase m-b-5 font-13 font-600"> Cam-1 / Cam-2 / Cam-3 / Cam-4</p>
                                        </div>
                                    </div><?php } ?>
                                </div>
                            </div>
...
<div align="center" style="display:none"><input type="text" id="Atmid" name="Atmid" placeholder="Search Atm-ID"  /><input type="button" value="search" onClick="a('','50')" /></div>
 ...
 $qrys=mysqli_query($conn,"select * from dvr_health where hdd='No Disk' or hdd='No disk/idle' ");
 }
 else if($value=="abnormal"){
 $qrys=mysqli_query($conn,"select * from dvr_health where hdd='abnormal' ");
 }
 else if($value=="notlogin"){
 $qrys=mysqli_query($conn,"select * from dvr_health where status='1' and login_status='1'  and live='y'   ");
 }
 else if($value=="Cam1"){
 $qrys=mysqli_query($conn,"select * from dvr_health where cam1='not working' and DATE(last_communication)<> '".$today."'");
 $qrys2=mysqli_query($conn,"select * from dvr_health where cam1='working' and DATE(last_communication)= '".$today."'");
 $iscam=1;
 }
 else if($value=="Cam2"){
 $qrys=mysqli_query($conn,"select * from dvr_health where cam2='not working' and DATE(last_communication)<> '".$today."'");
  $qrys2=mysqli_query($conn,"select * from dvr_health where cam2='working' and DATE(last_communication)= '".$today."'");
  $iscam=1;
 }
 else if($value=="Cam3"){
 $qrys=mysqli_query($conn,"select * from dvr_health where cam3='not working' and DATE(last_communication)<> '".$today."'");
  $qrys2=mysqli_query($conn,"select * from dvr_health where cam3='working' and DATE(last_communication)= '".$today."'");
  $iscam=1;
 }
 else if($value=="Cam4"){
 $qrys=mysqli_query($conn,"select * from dvr_health where cam4='not working' and DATE(last_communication)<> '".$today."'");
  $qrys2=mysqli_query($conn,"select * from dvr_health where cam4='working' and DATE(last_communication)= '".$today."'");
 $iscam=1;
 }
...
<?php 
 $sr=1 ; 
 while($row2 = mysqli_fetch_array($qrys2)) { 
$StateQry2= mysqli_query($conn,"select State,PanelIP,City,SiteAddress,Bank from sites where ATMID='".$row2['atmid']."'");
$fetchState2=mysqli_fetch_array($StateQry2);

$numQry2= mysqli_query($conn,"select id from dvr_health where status='1' and login_status='1' and id='".$row2['id']."' ");
$num=mysqli_num_rows($numQry2);


if($row2[11]!="0000-00-00 00:00:00"){
    $currdat=date("Y-m-d");
$date1=date_create($currdat);
$date2=date_create($row2[11]);
$diff=date_diff($date1,$date2);
$datedif_cnt=$diff->format("%a days");

}else{$datedif_cnt="NA";}
?>
  <tr <?php if($row2[2]==0 || $row2[10]==1){ ?>style="background-color:#FF0000" <?php } ?> >
    <td><?php echo $sr;?></td>
       <td><?php echo $row2[12];?></td>
       <td ><?php echo $row2[9];?></td>
       <td align="center"><div id=""><?php echo $row2[11]; ?></div></td>
       <td align="center"><div id=""><?php echo $datedif_cnt ; ?></div></td>
       <td align="center"><div id=""><?php echo $row2[1]; ?></div></td>
       <td align="center"><div id=""><?php if($row2[2]==1)echo 'good'; else echo 'bad'; ?></div></td>
       <td align="center"><div id=""><?php echo $row2[17]; ?></div></td>
       <td align="center"><div id=""><?php if($row2[2]==1 && $row2[10]==0)echo 'yes'; else echo 'No'; ?></div></td>
       <td align="center"><div id=""><?php echo $row2[3]; ?></div></td>
       <td align="center"><div id=""><?php echo $row2[4]; ?></div></td>
       <td align="center"><div id=""><?php echo $row2[5]; ?></div></td>
       <td align="center"><div id=""><?php echo $row2[6]; ?></div></td>
       <td align="center"><div id=""><?php echo $row2[7]; ?></div></td>
       <td align="center"><div id=""><?php echo $row2[15]; ?></div></td>
       <td align="center"><div id=""><?php echo $row2[13]; ?></div></td>
       <td align="center"><div id=""><?php echo $row2[14]; ?></div></td>
       <td align="center"><div id=""><?php echo $row2[8]; ?></div></td>
      <!-- <td align="center"><div id=""><?php if($num>0){ echo "Not Working"; }  ?></div></td>-->
       <td><?php echo $fetchState2['Bank'];?></td>
       <td ><?php echo $fetchState2['City'];?></td>
       <td ><?php echo $fetchState2['State'];?></td>
       <td ><?php echo $fetchState2['SiteAddress'];?></td> 
    </tr>
   <?php
$sr++;
