                        ...
                        <div class="card-box">
                            ...
                            <div class="text-center mt-4 mb-4">
                                <div class="row">
                                    ...
                                    <?php  if($value=="ConnSmartiHDFC" || $value=="NotConnSmartiHDFC"){  ?>
                                    <div class="col-md-6 col-xl-3">
                                        <div class="card-box bg-danger widget-flat border-danger text-white" style="padding-bottom: 0px;">
                                            <i class="fi-delete" style="pointer-events: none; cursor: default;">Securico</i>
                                             <h>Smart-I HDFC</h>
                                           <h3 class="m-b-10"><?php $Connectqry=mysqli_query($conn,"select count(*) as totalConnect from panel_health where status='0' and panelName ='smarti_hdfc32' ");
                                            $fetchConnectQry=mysqli_fetch_array($Connectqry);
                                            
                                            $NotConnectqry=mysqli_query($conn,"select count(*) as totalNotConnect from panel_health where status='1' and panelName ='smarti_hdfc32'");
                                            $fetchNotConnectQry=mysqli_fetch_array($NotConnectqry);
                                            
                                            echo $fetchConnectQry['totalConnect'] . "/" .  $fetchNotConnectQry['totalNotConnect'];
                                            ?></h3>
                                            <p class="text-uppercase m-b-5 font-13 font-600"  ><a href="Smarti_HDFC_Panel_POPup.php?id=ConnSmartiHDFC" style="color:white" target="_blank">Connected</a> / <a href="Smarti_HDFC_Panel_POPup.php?id=NotConnSmartiHDFC" style="color:white" target="_blank">Not Connected</a></p>
                                        </div>
                                    </div><?php } ?>
                                </div>
                            </div>
...
<div align="center" style="display:none"><input type="text" id="Atmid" name="Atmid" placeholder="Search Atm-ID"  /><input type="button" value="search" onClick="a('','50')" /></div>
     ...
     <td ><div >ATM-ID</div></td>
      ...
      <td ><div >Date_Time&nbsp; </div></td>
        <td ><div style="" id="">AC Mains Fails (DI) </div></td>
        <td><div style="" id="">UPS Out Fails (DI)</div></td>
        <td><div style="" id="">Hooter Sense (Sounder Trouble)</div></td>
        <td><div style="" id="">Panel Tamper + Vibration sensor</div></td>
        
        <td><div id="">Silence Key</div></td>
        <td><div id="">Smoke Detector I/P</div></td>
        <td><div style="" id="">Panic Switch</div></td>
        <td><div id="">Backroom Door Open Sensor</div></td>
        <td><div id="">Motion Sensor</div></td>
        <td><div id="">Glass Break Sensor</div></td>
        <td><div id="">ATM 1 Vibration Sensor</div></td>
      
        <td><div id="">ATM 1 Removal Sensor</div></td>
        <td><div id="">ATM 1 Thermal Sensor</div></td>
        <td><div id="">ATM 1 Chest Door Open Sensor</div></td>
        <td><div style="" id="">ATM Main Door Shutter Sensor</div></td>
        <td><div style="" id="">Cheque Drop Box Removal Sensor</div></td>
        <td><div id="">ATM 1 Hood Door Open</div></td>
        <td><div style="" id="">Speaker +MIC Removal Sensor</div></td>
        <td><div style="" id="">A/C 1/ 2 Removal Sensor</div></td>
        <td><div style="" id="">ATM Lobby Temperature</div></td>
      
        <td><div style="" id="">PIN PAD REMOVAL SENSOR</div></td>
        <td><div id="">SPARE</div></td>
        <td><div style="" id="">Hooter ON Locally</div></td>
        <td><div id="">HEARTBEAT </div></td>
        <td><div style="" id="">Network Alarm (Router reset RLY ON)</div></td>
        <td><div id="">Authorize card</div></td>
        <td><div id="">Unuthorize Card</div></td>
        <td><div style=""  id="">System Arm for Backroom</div></td>
...
<th>Relay 10</th>
 ...
 <?php
 $sr=1;
 if($value=="ConnSmartiHDFC" ){
 $qrys=mysqli_query($conn,"select * from panel_health where status='0' and panelName='smarti_hdfc32' ");
 }else if($value=="NotConnSmarti" || $value=="NotConnSmartiHDFC"){
    
 $qrys=mysqli_query($conn,"select * from panel_health where status='1' and panelName='smarti_hdfc32' ");
 }
...
<td align="center"><div id=""><?php if($row['relay1']==0){echo "Off";}else if($row['relay1']==1){echo "On";}else if($row['relay1']==2){echo "Disconnect";} ?></div></td>
<td align="center"><div id=""><?php if($row['relay2']==0){echo "Off";}else if($row['relay2']==1){echo "On";}else if($row['relay2']==2){echo "Disconnect";} ?></div></td>
<td align="center"><div id=""><?php if($row['relay3']==0){echo "Off";}else if($row['relay3']==1){echo "On";}else if($row['relay3']==2){echo "Disconnect";} ?></div></td>
<td align="center"><div id=""><?php if($row['relay4']==0){echo "Off";}else if($row['relay4']==1){echo "On";}else if($row['relay4']==2){echo "Disconnect";} ?></div></td>
<td align="center"><div id=""><?php if($row['relay5']==0){echo "Off";}else if($row['relay5']==1){echo "On";}else if($row['relay5']==2){echo "Disconnect";} ?></div></td>
<td align="center"><div id=""><?php if($row['relay6']==0){echo "Off";}else if($row['relay6']==1){echo "On";}else if($row['relay6']==2){echo "Disconnect";} ?></div></td>
<td align="center"><div id=""><?php if($row['relay7']==0){echo "Off";}else if($row['relay7']==1){echo "On";}else if($row['relay7']==2){echo "Disconnect";} ?></div></td>
<td align="center"><div id=""><?php if($row['relay8']==0){echo "Off";}else if($row['relay8']==1){echo "On";}else if($row['relay8']==2){echo "Disconnect";} ?></div></td>
<td align="center"><div id=""><?php if($row['relay9']==0){echo "Off";}else if($row['relay9']==1){echo "On";}else if($row['relay9']==2){echo "Disconnect";} ?></div></td>
<td align="center"><div id=""><?php if($row['relay10']==0){echo "Off";}else if($row['relay10']==1){echo "On";}else if($row['relay10']==2){echo "Disconnect";} ?></div></td>
