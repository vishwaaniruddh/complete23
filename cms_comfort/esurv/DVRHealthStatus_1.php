   ...
   $add=" and atmid IN (select ATMID from sites where Customer='Hitachi' and live='Y')";
                                    ...
                                    <div class="col-md-6 col-xl-3">
                                        <div class="card-box widget-flat border-success bg-success text-white" style="padding-bottom: 0px;">
                                            ...
                                            $Q8="select atmid from dvr_health where live='y' ";
                                            if($_SESSION['permission']!="" && $_SESSION['designation']=="2"){$Q8.= $add;}
											if($_SESSION['designation']=="4"){$Q8.= $add;}
                                            $qrydvr=mysqli_query($conn,$Q8);
											$totalDVRnotlogin = 0;
                                            while($fetchDvrQry=mysqli_fetch_assoc($qrydvr)){
												$_dvr_atmid = $fetchDvrQry['atmid'];
												$ch_dvr_his = "select id from dvr_history where atmid='".$_dvr_atmid."' AND login_status='0' AND CAST(last_communication AS DATE)='".$today."'";
											    $qrych_dvr_his=mysqli_query($conn,$ch_dvr_his);
												if(mysqli_num_rows($qrych_dvr_his)==0){
													$totalDVRnotlogin = $totalDVRnotlogin + 1;
												}
											}
                                            echo $totalDVRnotlogin;?></h3>
