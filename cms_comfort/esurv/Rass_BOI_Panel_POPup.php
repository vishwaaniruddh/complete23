                        ...
                        <div class="card-box">
                            ...
                            <div class="text-center mt-4 mb-4">
                                <div class="row">
                                    ...
                                    <?php  if($value=="ConnRassBOI" || $value=="NotConnRassBOI"){  ?>
                                    <div class="col-md-6 col-xl-3">
                                        <div class="card-box bg-danger widget-flat border-danger text-white" style="padding-bottom: 0px;">
                                            <i class="fi-delete" style="pointer-events: none; cursor: default;">RASS BOI</i>
                                             <h>RASS BOI</h>
                                           <h3 class="m-b-10"><?php $Connectqry=mysqli_query($conn,"select count(*) as totalConnect from panel_health where status='0' and panelName ='rass_boi' ");
                                            $fetchConnectQry=mysqli_fetch_array($Connectqry);
                                            
                                            $NotConnectqry=mysqli_query($conn,"select count(*) as totalNotConnect from panel_health where status='1' and panelName ='rass_boi'");
                                            $fetchNotConnectQry=mysqli_fetch_array($NotConnectqry);
                                            
                                            echo $fetchConnectQry['totalConnect'] . "/" .  $fetchNotConnectQry['totalNotConnect'];
                                            ?></h3>
                                            <p class="text-uppercase m-b-5 font-13 font-600"  ><a href="Rass_BOI_Panel_POPup.php?id=ConnRassBOI" style="color:white" target="_blank">Connected</a> / <a href="Rass_BOI_Panel_POPup.php?id=NotConnRassBOI" style="color:white" target="_blank">Not Connected</a></p>
                                        </div>
                                    </div><?php } ?>
                                </div>
                            </div>
...
<div align="center" style="display:none"><input type="text" id="Atmid" name="Atmid" placeholder="Search Atm-ID"  /><input type="button" value="search" onClick="a('','50')" /></div>
     ...
     <td ><div >ATM-ID</div></td>
      ...
      <td ><div >Date_Time&nbsp; </div></td>
        ...
        <td><div style="" id="">Main Door shutter Normally NO type</div></td>
        ...
        <td><div id="">Speaker / MIC Removal</div></td>
        <td><div id="">ATM 1 Thermal/ Heat</div></td>
        <td><div id="">ATM 2 Thermal/ Heat</div></td>
        <td><div style="" id="">Chest Door Open ATM1</div></td>
        <td><div style="" id="">Chest Door Open ATM2</div></td>
        <td><div id="">Chest Door Open ATM3</div></td>
        <td><div style="" id="">AC /UPS removal</div></td>
        <td><div style="" id="">Cheque drop box removal</div></td>
        <td><div style="" id="">Cheque drop box open</div></td>
        ...
        <td><div style="" id="">ATM 1/2 Hood Door Sensor</div></td>
 ...
 if($value=="ConnRassBOI" ){
 $qrys=mysqli_query($conn,"select * from panel_health where status='0' and panelName='rass_boi' ");
 }else if($value=="NotConnSmartIN"){
 $qrys=mysqli_query($conn,"select * from panel_health where status='1' and panelName='rass_boi' ");
 }
