...
$sql = mysqli_query($conn,"select * from all_dvr_live");
while ($row = mysqli_fetch_array($sql)) {
  ...
  if($atmid){
    $sitesql = mysqli_query($conn,"select * from sites where ATMID='".$atmid."' or ATMID_2='".$atmid."' 
    or ATMID_3='".$atmid."' or ATMID_4='".$atmid."'");
    $site = mysqli_fetch_array($sitesql);
    $sn = $site['SN'];
  
    $update = "update all_dvr_live set SN='".$sn."' where id='".$id."'" ;
   ...
   $add=" and atmid IN (select ATMID from sites where Customer='Hitachi' and live='Y')";
                                    ...
                                    <div class="col-md-6 col-xl-3">
                                        <div class="card-box bg-primary widget-flat border-primary text-white" style="padding-left: 4px;padding-bottom: 0px;">
                                            ...
                                            echo
                                             ...
                                             '<span id="totalErrorCount">'.$fetcherrorQry['totalError'].'</span>'.
                                              " / " .
                                              '<span id="totalNotExist">'.$fetchNotExistQry['totalNotExist'].'</span>'.
                                                 " / " .
                                              '<span id="totalSmartFailed">'.$fetchsmartFailedQry['totalSmartFailed'].'</span>'.
                                                " / " . 
                                              '<span id="totalUnformatted">'.$fetchunformattedQry['totalUnformatted'].'</span>'.


                                               "</br> / " . $fetchabNormalQry['totalAbNormal'] ." / " . $fetchnoDiskQry['totalNoDisk'];
                                            } 
											?></h3>
                                    ...
                                    <div class="col-md-6 col-xl-3">
                                        <div class="card-box bg-danger widget-flat border-danger text-white" style="padding-bottom: 0px;">
                                            ...
                                            <p class="text-uppercase m-b-5 font-13 font-600"> <a href="DVR_POPup.php?id=Cam1" style="color:white" target="_blank">Cam-1</a> / <a href="DVR_POPup.php?id=Cam2" style="color:white" target="_blank">Cam-2</a> 
											<?php  if($_SESSION['designation']!="4"){ ?>
											/ <a href="DVR_POPup.php?id=Cam3" style="color:white" target="_blank">Cam-3</a>/ <a href="DVR_POPup.php?id=Cam4" style="color:white" target="_blank">Cam-4</a></p>
                                            <?php }?>
										</div>
                                    </div>
                                </div>
