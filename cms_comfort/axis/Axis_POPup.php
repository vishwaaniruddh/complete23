                        ...
                        <div class="card-box">
                            ...
                            <div class="text-center mt-4 mb-4">
                                <div class="row">
                                     ...
                                     <?php if($value=="Conn" || $value=="NotConn"){?>
                                    <div class="col-md-6 col-xl-3">
                                        <div class="card-box widget-flat border-custom bg-custom text-white" style="padding-bottom: 0px;padding-top: 9px;">
                                            ...
                                            <?php $qrydvr=mysqli_query($conn,"select count(*) as totalDVR from axisdvr_health where  dvrtype='Hikvision'  ");
                                            $fetchDvrQry=mysqli_fetch_array($qrydvr);
                                            echo $fetchDvrQry['totalDVR'];
                                            ?></h4>

                                            <h3 class="m-b-10" ><?php $Connectqry=mysqli_query($conn,"select count(*) as totalConnect from axisdvr_health where status='1' and dvrtype='Hikvision' and live='y' ");
                                            $fetchConnectQry=mysqli_fetch_array($Connectqry);
                                            
                                            $NotConnectqry=mysqli_query($conn,"select count(*) as totalNotConnect from axisdvr_health where `status`='0' and dvrtype='Hikvision' and live='y' ");
                                     ...
                                     <?php }else if($value=="error"||$value=="NotExist"||$value=="SmartFail"||$value=="UnFormated"){  ?>
                                    ...
                                    <div class="col-md-6 col-xl-3">
                                        <div class="card-box bg-primary widget-flat border-primary text-white" style="padding-left: 4px;padding-bottom: 0px;">
                                            ...
                                            <h>HDD Not Working</h> 
                                            <h3 class="m-b-10"><?php $errorqry=mysqli_query($conn,"select count(*) as totalError from axisdvr_health where hdd='error'");
                                            $fetcherrorQry=mysqli_fetch_array($errorqry);
                                            
                                            $notexistqry=mysqli_query($conn,"select count(*) as totalNotExist from axisdvr_health where hdd='notexist' ");
                                            $fetchNotExistQry=mysqli_fetch_array($notexistqry);
                                            
                                            $smartFailedqry=mysqli_query($conn,"select count(*) as totalSmartFailed from axisdvr_health where hdd='smartFailed' ");
                                            $fetchsmartFailedQry=mysqli_fetch_array($smartFailedqry);
                                            
                                            $unformattedqry=mysqli_query($conn,"select count(*) as totalUnformatted from axisdvr_health where hdd='unformatted' ");
                                    ...
                                    <div class="col-md-6 col-xl-3">
                                        <div class="card-box widget-flat border-success bg-success text-white" style="padding-bottom: 0px;">
                                            ...
                                            <h>DVR Status</h>
                                            <h3 class="m-b-10"><?php $qrydvr=mysqli_query($conn,"select count(*) as totalDVR from axisdvr_health where status='1' and login_status='1' ");
                                            $fetchDvrQry=mysqli_fetch_array($qrydvr);
                                    ...
                                    <?php }else if($value=="Cam1" || $value=="Cam2" || $value=="Cam3"|| $value=="Cam4"){  ?>
                                    <div class="col-md-6 col-xl-3">
                                        <div class="card-box bg-danger widget-flat border-danger text-white" style="padding-bottom: 0px;">
                                            ...
                                            <h3 class="m-b-10">
                                                <?php 
                                            $Notworkingqry1=mysqli_query($conn,"select count(*) as totalNotworking from axisdvr_health where cam1='not working'  ");
                                            $fetchNotworkingQry1=mysqli_fetch_array($Notworkingqry1);
                                            
                                           
                                            $Notworkingqry2=mysqli_query($conn,"select count(*) as totalNotworking from axisdvr_health where cam2='not working' ");
                                            $fetchNotworkingQry2=mysqli_fetch_array($Notworkingqry2);
                                            
                                            
                                            $Notworkingqry3=mysqli_query($conn,"select count(*) as totalNotworking from axisdvr_health where cam3='not working'  ");
                                            $fetchNotworkingQry3=mysqli_fetch_array($Notworkingqry3);
                                            
                                            $Notworkingqry4=mysqli_query($conn,"select count(*) as totalNotworking from axisdvr_health where cam4='not working'  ");
                                         ...
                                         <p class="text-uppercase m-b-5 font-13 font-600"><a href="Axis_POPup.php?id=Cam1" style="color:white" target="_blank">Cam-1</a>/<a href="Axis_POPup.php?id=Cam2" style="color:white" target="_blank">Cam-2</a>/<a href="Axis_POPup.php?id=Cam3" style="color:white" target="_blank">Cam-3</a>/<a href="Axis_POPup.php?id=Cam4" style="color:white" target="_blank">Cam-4</a> </p>
                                          </div>
                                    </div><?php } ?>
                                </div>
                            </div>
...
<div align="center" style="display:none"><input type="text" id="Atmid" name="Atmid" placeholder="Search Atm-ID"  /><input type="button" value="search" onClick="a('','50')" /></div>
 ...
 if($value=="Conn" ){
 $qrys=mysqli_query($conn,"select * from axisdvr_health where status='1' and dvrtype='Hikvision' and live='y' ");
 }else if($value=="NotConn"){
 $qrys=mysqli_query($conn,"select * from axisdvr_health where status='0' and dvrtype='Hikvision' and live='y'  ");
 }
 else if($value=="error"){
 $qrys=mysqli_query($conn,"select * from axisdvr_health where hdd='error' ");
 }
 else if($value=="NotExist"){
 $qrys=mysqli_query($conn,"select * from axisdvr_health where hdd='notexist'");
 }
 else if($value=="SmartFail"){
 $qrys=mysqli_query($conn,"select * from axisdvr_health where hdd='smartFailed'  ");
 }
 else if($value=="UnFormated"){
 $qrys=mysqli_query($conn,"select * from axisdvr_health where hdd='unformatted'  ");
 }
 else if($value=="notlogin"){
 $qrys=mysqli_query($conn,"select * from axisdvr_health where status='1' and login_status='1'    ");
 }
 else if($value=="Cam1"){
 $qrys=mysqli_query($conn,"select * from axisdvr_health where cam1='not working'   ");
 }
 else if($value=="Cam2"){
 $qrys=mysqli_query($conn,"select * from axisdvr_health where cam2='not working'    ");
 }
  else if($value=="Cam3"){
 $qrys=mysqli_query($conn,"select * from axisdvr_health where cam3='not working'    ");
 }
  else if($value=="Cam4"){
 $qrys=mysqli_query($conn,"select * from axisdvr_health where cam4='not working'    ");
 }
