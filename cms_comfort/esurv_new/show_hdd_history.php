	...
	<table class="table table-bordered table-striped table-hover dataTable js-exportable no-footer" style="width:100%">
		<thead>
			<tr>
				<th>S.No</th>
				<td>ATM ID</td>
				<td>Last HDD Working Communication</td>
				<td>Last HDD Not Working Communication</td>
		...
		<tbody>
			<?php  // $dvr_statement = "select * from dvr_health where SN=".$sn." order by id desc" ;
                   ...
                   // $sql_result = mysqli_fetch_assoc($dvrhealth_sql);   
                    $query_nc = "SELECT * FROM `dvr_history` WHERE atmid='".$atmid."' AND (hdd!='OK' and hdd!='ok') ORDER BY id DESC LIMIT 1";  
                    $query_ok = "SELECT * FROM `dvr_history` WHERE atmid='".$atmid."' AND (hdd='OK' or hdd='ok') ORDER BY id DESC LIMIT 1"; 					
