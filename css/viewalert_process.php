...
//   echo "select TestingByService,remark from Testing_alertDetails where incident_id='".$row["id"]."' ";
...
$sql1="select sensorname as Description,camera from securico where (Zone='".$row["zone"]."')";
	...
	*/
	...
	else if($row["Panel_make"]== "smarti_pnb"){		 
     $sql1="select SensorName as Description,Camera from smarti_pnb where (Zone='".$row["zone"]."' and SCODE='".$row['alarm']."')";
	}
	else if($row["Panel_make"]== "rass_boi"){		 
     $sql1="select SensorName as Description,Camera from rass_boi where (Zone='".$row["zone"]."' and SCODE='".$row['alarm']."')";
	}
